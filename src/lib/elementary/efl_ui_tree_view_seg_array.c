   ...
   while (i < node->length)
     {
        ...
        if (item->segarray)
          {
             efl_ui_tree_view_seg_array_flush(item->segarray);
             efl_unref(item->segarray);
          }
...
_efl_ui_tree_view_seg_array_insert(Eo *obj EINA_UNUSED, Efl_Ui_Tree_View_Seg_Array_Data* pd, Efl_Ui_Tree_View_Item* parent, int shallow_index, Efl_Model* model)
{
   ...
   Efl_Ui_Tree_View_Seg_Array_Data* priv = pd;
   Efl_Ui_Tree_View_Item* item;
   Eina_Iterator* iterator;
   ...
   if (parent)
     {
        if (!parent->segarray)
          parent->segarray = efl_new(EFL_UI_TREE_VIEW_SEG_ARRAY_CLASS,
                              efl_ui_tree_view_seg_array_setup(efl_added, pd->step_size));

        _efl_ui_tree_view_seg_array_fix_offset (parent);
        pd->count++;
        priv = efl_data_scope_get(parent->segarray, EFL_UI_TREE_VIEW_SEG_ARRAY_CLASS);
     }
