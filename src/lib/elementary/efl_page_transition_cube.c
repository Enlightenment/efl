...
#include "efl_page_transition_cube.h"
...
static void
_cube_effect(Evas_Object *obj, Evas_Object *target, double t, int c)
{
   ...
   evas_map_util_3d_rotate(map, 0.0, deg, 0.0, cx, cy, sd->w / 2);
...
EOLIAN static void
_efl_page_transition_cube_efl_page_transition_update(Eo *obj EINA_UNUSED,
                                                     Efl_Page_Transition_Cube_Data *_pd EINA_UNUSED,
                                                     ...
                                                     double position)
{
   ...
   if (position <= 0) _cube_effect(parent, target, (-1) * position, 1);
   else _cube_effect(parent, target, position, 2);
}

#include "efl_page_transition_cube.eo.c"
