...
_edj_marker_size_get(Widget_Data *wd, Evas_Coord *w, Evas_Coord *h)
{
   ...
   _elm_theme_object_set(wd->obj, edj, "map/marker", "radio", elm_widget_style_get(wd->obj));
...
_marker_place(Widget_Data *wd)
{
   ...
   if (wd->paused_markers || eina_list_count(wd->markers) == 0) return;
...
_download_job(void *data)
{
   ...
   if (eina_list_count(wd->download_list) == 0)
     {
...
_nominatim_url_cb(Evas_Object *obj, int method, char *name, double lon, double lat)
{
   ...
   else if (method == ELM_MAP_NAME_METHOD_REVERSE) snprintf(buf, sizeof(buf), "%s/reverse?format=xml&lat=%lf&lon=%lf&zoom=%d&addressdetails=0", NAME_NOMINATIM_URL, lat, lon, (int)wd->zoom);
   else strcpy(buf, "");
...
elm_map_canvas_to_geo_convert(const Evas_Object *obj, Evas_Coord x, Evas_Coord y, double *lon, double *lat)
{
   ...
   _pan_geometry_get(wd, &px, &py);
   _viewport_size_get(wd, &vw, &vh);
   _coord_rotate(x - px, y - py, vw/2 - px, vh/2 - py, -wd->rotate.d, &x, &y);
...
#ifdef HAVE_ELEMENTARY_ECORE_CON
   EINA_SAFETY_ON_NULL_RETURN(markers);
   EINA_SAFETY_ON_TRUE_RETURN(eina_list_count(markers) == 0);
