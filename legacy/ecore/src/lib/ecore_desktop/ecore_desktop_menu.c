...
static int _ecore_desktop_menus_unxml(const void *data, Ecore_Desktop_Tree * tree, int element, int level);
static int _ecore_desktop_menus_check_directory(const void *data, char *path);
static int _ecore_desktop_menus_check_menu(const void *data, char *path);
static int _ecore_desktop_menus_legacy_menu_dir(const void *data, char *path);
static int _ecore_desktop_menus_legacy_menu(const void *data, char *path);
static void _ecore_desktop_menus_unxml_rules(Ecore_Desktop_Tree * rules, Ecore_Desktop_Tree * tree, char type, char sub_type);
static void _ecore_desktop_menus_unxml_moves(Ecore_Desktop_Tree * menu, Ecore_Desktop_Tree * tree);
static void _ecore_desktop_menus_add_dirs(Ecore_Desktop_Tree * tree, Ecore_List * paths, char *pre, char *post, char *extra, int element);
static int _ecore_desktop_menus_expand_apps(struct _ecore_desktop_menus_unxml_data *unxml_data, char *app_dir, Ecore_Hash * pool);
static int _ecore_desktop_menus_check_app(const void *data, char *path);

static int _ecore_desktop_menus_merge(const void *data, Ecore_Desktop_Tree * tree, int element, int level);
static int _ecore_desktop_menus_expand_default_dirs(const void *data, Ecore_Desktop_Tree * tree, int element, int level);

static int _ecore_desktop_menus_generate(const void *data, Ecore_Desktop_Tree * tree, int element, int level);
static void _ecore_desktop_menus_inherit_apps(void *value, void *user_data);
static void _ecore_desktop_menus_select_app(void *value, void *user_data);
static int _ecore_desktop_menus_apply_rules(struct _ecore_desktop_menus_generate_data *generate_data, Ecore_Desktop_Tree * rule, char *key, Ecore_Desktop * desktop);

Ecore_Desktop_Tree *
ecore_desktop_menus_get(char *file, Ecore_Desktop_Tree * merge_stack, int level)
{
   ...
   if ((data.stack) && (data.base) && (data.path) && (merge_stack) && (menu_xml))
     {
        ...
        if (oops == 0)
          {
             /* Get on with it. */
             ecore_desktop_tree_foreach(menu_xml, 0, _ecore_desktop_menus_unxml, &data);
             ecore_desktop_tree_foreach(menu_xml, 0, _ecore_desktop_menus_merge, &data);

             /* The rest of this is only done after ALL the menus have been merged. */
             if (level == 0)
               {
                  ecore_desktop_tree_foreach(menu_xml, 0, _ecore_desktop_menus_expand_default_dirs, &data);
                  ...
                  ecore_desktop_tree_foreach(menu_xml, 0, _ecore_desktop_menus_generate, &data);
...
static int
_ecore_desktop_menus_unxml(const void *data, Ecore_Desktop_Tree * tree, int element, int level)
{
             ...
             if (menu)
               {
                  ...
                  tree->elements[element].type = ECORE_DESKTOP_TREE_ELEMENT_TYPE_TREE;
                       ...
                       if (tree->elements[i].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_STRING)
                         {
                            if (strncmp((char *)tree->elements[i].element, "<!", 2) == 0)
                              ...
                              {
                                 ecore_desktop_tree_extend(menu, (char *)tree->elements[i].element);
                                 result = 1;
                              }
                         }
                                 ...
                                 if (sub->elements[0].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_STRING)
                                   {
                                      if (strcmp((char *)sub->elements[0].element, "<Name") == 0)
                                        {
                                           ...
                                           ecore_desktop_tree_track(menu, menu->elements[0].element);
                                           flags = (char *)menu->elements[0].element;
                                           flags += 7;
                                           ...
                                           if (unxml_data->stack->size <= level)
                                             {
                                                while (unxml_data->stack->size < level)
                                                   ecore_desktop_tree_add(unxml_data->stack, "");
                                                ecore_desktop_tree_add(unxml_data->stack, name);
                                             }
                                           else
                                           ...
                                           ecore_desktop_tree_track(menu, menu_path);
                                           ecore_desktop_tree_track(menu, menu->elements[1].element);
                                           result = 1;
                                        }
                                      /* FIXME: Move this to later in the sequence. */
                                      else if (strcmp((char *)sub->elements[0].element, "<Directory") == 0)
                                        {
                                           ...
                                           sprintf(temp, "<MENU <%.4s> <%s> <%s>", flags, name, directory);
                                           menu->elements[0].element = strdup(temp);
                                           ecore_desktop_tree_track(menu, directory);
                                           ecore_desktop_tree_track(menu, menu->elements[0].element);
                                           flags = (char *)menu->elements[0].element;
                                           flags += 7;
                                           result = 1;
                                        ...
                                        {
                                           _ecore_desktop_menus_unxml(data, sub, 0, level + 1);
                                           ecore_desktop_tree_add_child(menu, (Ecore_Desktop_Tree *) sub->elements[0].element);
...
//                                                 result = 1;
                                        ...
                                        {
                                           _ecore_desktop_menus_unxml_moves(menu, sub);
                                           result = 1;
                                        }
                                      else
                                        {
                                           if ((sub->size == 3) && (sub->elements[1].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_STRING)
                                             ...
                                             {
                                                ecore_desktop_tree_add_child(menu, sub);
                                                tree->elements[i].type = ECORE_DESKTOP_TREE_ELEMENT_TYPE_NULL;
                                                tree->elements[i].element = NULL;
                                             }
                                        }
	          ...
	          if (menu)
	            {
                       ...
                       menu->elements[0].element = strdup(temp);
                       ecore_desktop_tree_track(menu, menu->elements[0].element);
                       sprintf(temp, "<MENU_PATH %s/%s", legacy_data->menu, &path[legacy_data->length]);
                       menu->elements[1].element = strdup(temp);
                       ecore_desktop_tree_track(menu, menu->elements[1].element);
...
_ecore_desktop_menus_legacy_menu(const void *data, char *path)
{
   ...
   if ((legacy_data->current) && (legacy_data->current->size > 0) &&
       (legacy_data->current->elements[1].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_STRING))
     {
   ...
   if (strcmp(".directory", file) == 0)
	...
	menu_path[menu_count - 3] = '\0';
        ...
        ecore_desktop_tree_track(legacy_data->current, legacy_data->current->elements[0].element);
     }
	...
	Ecore_Desktop_Tree *rules;
        ...
        rules = (Ecore_Desktop_Tree *) legacy_data->current->elements[3].element;
	if (rules->size == 0)
	  {
...
static void
_ecore_desktop_menus_unxml_rules(Ecore_Desktop_Tree * rules, Ecore_Desktop_Tree * tree, char type, char sub_type)
{
   int i;
   char temp[MAX_PATH];
        ...
        if (tree->elements[i].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_STRING)
          {
             if (strcmp((char *)tree->elements[i].element, "<All/") == 0)
               ...
               {
                  _ecore_desktop_menus_unxml_rules(rules, (Ecore_Desktop_Tree *) tree->elements[i + 1].element, type, sub_type);
               }
                  ...
                  if (sub)
                    {
                       ...
                       for (i++; i < tree->size; i++)
                         {
                            if (tree->elements[i].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_TREE)
                               _ecore_desktop_menus_unxml_rules(sub, (Ecore_Desktop_Tree *) tree->elements[i].element, type, this_type);
                         }
                    }
               }
          }
          ...
          {
             _ecore_desktop_menus_unxml_rules(rules, (Ecore_Desktop_Tree *) tree->elements[i].element, type, sub_type);
          }
     }
}

static void
_ecore_desktop_menus_unxml_moves(Ecore_Desktop_Tree * menu, Ecore_Desktop_Tree * tree)
{
   int i;
   char *old = NULL;
   char *new = NULL;
...
static void
_ecore_desktop_menus_add_dirs(Ecore_Desktop_Tree * tree, Ecore_List * paths, char *pre, char *post, char *extra, int element)
{
...
static int
_ecore_desktop_menus_expand_apps(struct _ecore_desktop_menus_unxml_data *unxml_data, char *app_dir, Ecore_Hash * pool)
{
   if (pool)
     {
        ...
        ecore_desktop_paths_recursive_search(dir, NULL, NULL, _ecore_desktop_menus_check_app, &our_data);
     }
   return 0;
}
...
static int
_ecore_desktop_menus_merge(const void *data, Ecore_Desktop_Tree * tree, int element, int level)
{
   struct _ecore_desktop_menus_unxml_data *unxml_data;
   Ecore_Desktop_Tree *merge;
   int result = 0;
   ...
   if (tree->elements[element].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_STRING)
     {
        ...
        if (strcmp(string, "<DefaultMergeDirs/") == 0)
          {
             if (unxml_data->base)
                _ecore_desktop_menus_add_dirs(merge, ecore_desktop_paths_menus, "<MergeDir", "</MergeDir", unxml_data->base, element);
             result = 1;
          }
        else if (strcmp(string, "<KDELegacyDirs/") == 0)
          {
             _ecore_desktop_menus_add_dirs(merge, ecore_desktop_paths_kde_legacy, "<LegacyDir prefix=\"kde-\"", "</LegacyDir", NULL, element);
             result = 1;
          }
        else if (strncmp(string, "<MergeDir ", 10) == 0)
          {
             ...
             ecore_desktop_paths_recursive_search(merge_path, NULL, NULL, _ecore_desktop_menus_check_menu, merge);
             result = 1;
          }
	     ...
	     legacy_data.menu += 14;
	     legacy_data.menu_length = index(legacy_data.menu, '>') - legacy_data.menu;
...
#endif
             ecore_desktop_paths_recursive_search(merge_path, NULL, _ecore_desktop_menus_legacy_menu_dir, _ecore_desktop_menus_legacy_menu, &legacy_data);
             ...
             if (merge_path[0] != '\0')
               {
                  ...
                  new_menu = ecore_desktop_menus_get(merge_path, unxml_data->merge_stack, level + 1);
                  if (new_menu)
                    {
                       if (new_menu->size > 1)
                         {
                            if (new_menu->elements[1].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_TREE)
                              {
                                 new_menu = (Ecore_Desktop_Tree *) new_menu->elements[1].element;
                                 if (new_menu->size > 0)
                                   {
                                      if (new_menu->elements[0].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_TREE)
                                        {
                                           merge = (Ecore_Desktop_Tree *) new_menu->elements[0].element;
                                         ...
                                         printf("FUCK an error in _ecore_desktop_menus_merge(%s)\n", merge_path);
                                   }
                                 else
                                    printf("FUCK another error in _ecore_desktop_menus_merge(%s)\n", merge_path);
                              }
                            else
                               printf("FUCK ME! An error in _ecore_desktop_menus_merge(%s)\n", merge_path);
                         }
                    }
               }
             result = 1;
...
static int
_ecore_desktop_menus_expand_default_dirs(const void *data, Ecore_Desktop_Tree * tree, int element, int level)
{
   struct _ecore_desktop_menus_unxml_data *unxml_data;
   Ecore_Desktop_Tree *merge;
   int result = 0;
   ...
   if (tree->elements[element].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_STRING)
     {
        ...
        if (strcmp(string, "<DefaultAppDirs/") == 0)
          {
             _ecore_desktop_menus_add_dirs(merge, ecore_desktop_paths_desktops, "<AppDir", "</AppDir", NULL, element);
             result = 1;
          }
        else if (strcmp(string, "<DefaultDirectoryDirs/") == 0)
          {
             _ecore_desktop_menus_add_dirs(merge, ecore_desktop_paths_directories, "<DirectoryDir", "</DirectoryDir", NULL, element);
             result = 1;
          }
     }
   if (result)
...
static int
_ecore_desktop_menus_generate(const void *data, Ecore_Desktop_Tree * tree, int element, int level)
{
   struct _ecore_desktop_menus_unxml_data *unxml_data;
                       ...
                       if (tree->elements[i].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_STRING)
                         {
                            ...
                            if (strncmp(string, "<AppDir ", 8) == 0)
                              {
                                 _ecore_desktop_menus_expand_apps(unxml_data, &string[8], generate_data.pool);
                                 result = 1;
                              }
                            else if (strncmp(string, "<DirectoryDir ", 14) == 0)
                              {
                                 ...
                                 if (merge)
                                   {
                                      ecore_desktop_paths_recursive_search(merge_path, NULL, NULL, _ecore_desktop_menus_check_directory, merge);
                                           ...
                                           if (new_sub)
                                             {
                                                ...
                                                _ecore_desktop_menus_unxml_rules(new_sub, sub, string[1], 'O');
                                             }
                                           result = 1;
                                        }
                                   }
                       ...
                       if (unxml_data->stack->elements[i].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_HASH)
                         {
                            ...
                            ecore_hash_for_each_node(ancestor, _ecore_desktop_menus_inherit_apps, generate_data.pool);
                         }
                    }
               }
                                 ...
                                 if (((char *)generate_data.rule->elements[0].element)[0] == 'I')
                                   {
                                      ...
                                      ecore_hash_for_each_node(generate_data.pool, _ecore_desktop_menus_select_app, &generate_data);
                                   }
                                 else
                                   {
                                      generate_data.include = FALSE;
                                      ecore_hash_for_each_node(generate_data.apps, _ecore_desktop_menus_select_app, &generate_data);
                                   }
                              }
                         }
                       else
...
_ecore_desktop_menus_select_app(void *value, void *user_data)
{
   ...
   if (_ecore_desktop_menus_apply_rules(generate_data, generate_data->rule, key, desktop))
     {
...
static int
_ecore_desktop_menus_apply_rules(struct _ecore_desktop_menus_generate_data *generate_data, Ecore_Desktop_Tree * rule, char *key, Ecore_Desktop * desktop)
{
   char type = 'O';
   int result = FALSE;
   int i;
   ...
   for (i = 0; i < rule->size; i++)
     {
        if (rule->elements[i].type == ECORE_DESKTOP_TREE_ELEMENT_TYPE_TREE)
          {
             result = _ecore_desktop_menus_apply_rules(generate_data, (Ecore_Desktop_Tree *) rule->elements[i].element, key, desktop);
          }
        else
          {
             char *rul;
