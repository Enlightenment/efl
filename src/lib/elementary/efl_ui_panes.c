   ...
   else
     {
        if (edje_object_part_exists
          (wd->resize_obj, "event"))
          {
             ...
             elm_coords_finger_size_adjust(1, &minw, 1, &minh);
             evas_object_size_hint_min_set(sd->event, minw, minh);
             elm_layout_content_set(obj, "event", sd->event);
          }
      }
...
_efl_ui_panes_split_ratio_get(const Eo *obj, Efl_Ui_Panes_Data *sd)
{
   ...
   if (elm_widget_is_legacy(obj))
     ...
     edje_object_part_drag_value_get(wd->resize_obj, "bar", &w, &h);
   ...
   if (sd->dir == EFL_UI_DIR_HORIZONTAL)
     {
        if (elm_widget_is_legacy(obj))
          ...
          edje_object_part_drag_value_set(wd->resize_obj, "bar", 0.0, ratio);
     }
   else
     {
        if (elm_widget_is_legacy(obj))
          ...
          edje_object_part_drag_value_set(wd->resize_obj, "bar", ratio, 0.0);
     }
}
