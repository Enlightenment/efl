...
#define MY_CLASS EFL2_CANVAS_TEXT_CLASS
...
// FIXME: Remove unused types and rename from evas_object
...
struct _Evas_Textblock_Node_Format
{
   ...
   Efl2_Text_Attribute_Handle         *annotation; /**< Pointer to this node's annotation handle (if exists). */
...
struct _Evas_Object_Textblock
{
   ...
   Efl2_Text_Attribute_Handle         *annotations; /* All currently applied annotations on the text. */
   int                                 last_w, last_h;
   struct {
      int                              l, r, t, b;
   } style_pad;
...
static void _evas_textblock_cursors_update_offset(const Efl2_Text_Cursor_Handle *cur, const Evas_Object_Textblock_Node_Text *n, size_t start, int offset);
...
static void _evas_textblock_annotation_remove(Efl2_Canvas_Text_Data *o, Efl2_Text_Attribute_Handle *an, Eina_Bool remove_nodes);
...
/**
  ...
  */
Eina_Iterator *
_canvas_text_selection_iterator_new(Eina_List *list)
{
...
_layout_done(Ctxt *c, Evas_Coord *w_ret, Evas_Coord *h_ret)
{
   ...
   if ((c->o->style_pad.l != c->style_pad.l) || (c->o->style_pad.r != c->style_pad.r) ||
       (c->o->style_pad.t != c->style_pad.t) || (c->o->style_pad.b != c->style_pad.b))
     {
        ...
        efl_event_callback_call(c->obj, EFL2_CANVAS_TEXT_EVENT_STYLE_INSETS_CHANGED, NULL);
...
EOLIAN static void
_efl2_canvas_text_newline_as_paragraph_separator_set(Eo *eo_obj EINA_UNUSED, Efl2_Canvas_Text_Data *o, Eina_Bool mode)
{
   Evas_Object_Protected_Data *obj = efl_data_scope_get(eo_obj, EFL_CANVAS_OBJECT_CLASS);
...
EOLIAN static Eina_Bool
_efl2_canvas_text_newline_as_paragraph_separator_get(const Eo *eo_obj EINA_UNUSED, Efl2_Canvas_Text_Data *o)
{
 ...
 */
static void
_evas_textblock_cursor_nodes_merge(Efl2_Text_Cursor_Handle *cur)
{
...
_evas_textblock_cursor_node_format_at_pos_get(const Efl2_Text_Cursor_Handle *cur)
{
 ...
 */
static Evas_Object_Textblock_Node_Format *
_evas_textblock_cursor_node_format_before_or_at_pos_get(const Efl2_Text_Cursor_Handle *cur)
{
...
void
_canvas_text_cursor_init(Efl2_Text_Cursor_Handle *cur, const Evas_Object *tb)
{
   cur->obj = (Eo *) tb;
 ...
 */
static Eina_Bool
_find_layout_item_match(const Efl2_Text_Cursor_Handle *cur, Evas_Object_Textblock_Line **lnr, Evas_Object_Textblock_Item **itr)
{
   ...
   if (_canvas_text_cursor_is_at_the_end(cur) &&
...
_canvas_text_cursor_paragraph_first(Efl2_Text_Cursor_Handle *cur)
{
   ...
   if ((cur->node == o->text_nodes) && (cur->pos == 0))
     {
        return EINA_FALSE;
     }
   cur->node = o->text_nodes;
...
Eina_Bool
_canvas_text_cursor_paragraph_last(Efl2_Text_Cursor_Handle *cur)
{
   Evas_Object_Textblock_Node_Text *node;
...
Eina_Bool
_canvas_text_cursor_paragraph_next(Efl2_Text_Cursor_Handle *cur)
{
   if (!cur) return EINA_FALSE;
...
Eina_Bool
_canvas_text_cursor_paragraph_prev(Efl2_Text_Cursor_Handle *cur)
{
   Evas_Object_Textblock_Node_Text *node;
   if (!cur) return EINA_FALSE;
   TB_NULL_CHECK(cur->node, EINA_FALSE);
...
size_t
_canvas_text_cursor_cluster_post_get(Efl2_Text_Cursor_Handle *cur, Eina_Bool inc)
{
   Evas_Object_Textblock_Paragraph *par;
   Efl2_Canvas_Text_Data *o;
...
Eina_Bool
_canvas_text_cursor_paragraph_start(Efl2_Text_Cursor_Handle *cur)
{
   if (cur->pos == 0) {
        return EINA_FALSE;
   }
...
Eina_Bool
_canvas_text_cursor_paragraph_end(Efl2_Text_Cursor_Handle *cur)
{
   int ind;
...
static void
_canvas_text_cursor_line_first_char_get(Evas_Object_Textblock_Line *ln,
...
Eina_Bool
_canvas_text_cursor_line_start(Efl2_Text_Cursor_Handle *cur)
{
   Evas_Object_Textblock_Line *ln = NULL;
   Evas_Object_Textblock_Item *it = NULL;
   if (!cur) return EINA_FALSE;
   // FIXME: This one should actually block, missing the ASYNC_BLOCK macro though? Why is this even split?
   ...
   _canvas_text_cursor_line_first_char_get(ln, cur, o);

   // FIXME: only when moved
   return EINA_TRUE;
}

Eina_Bool
_canvas_text_cursor_line_end(Efl2_Text_Cursor_Handle *cur)
{
   Evas_Object_Textblock_Line *ln = NULL;
   Evas_Object_Textblock_Item *it = NULL;
...
static Eina_Bool
_evas_textblock_cursor_is_format(const Efl2_Text_Cursor_Handle *cur)
{
...
_evas_textblock_cursor_node_text_at_format(Efl2_Text_Cursor_Handle *cur, Evas_Object_Textblock_Node_Format *fmt)
{
                  ...
                  EINA_LIST_FOREACH_SAFE(formats, i, next, fnode)
                    {
                       if (_FORMAT_IS_CLOSER_OF(
                         ...
                         {
                            Efl2_Text_Attribute_Handle *an = fmt->annotation;
...
int
_canvas_text_cursor_position_get(const Efl2_Text_Cursor_Handle *cur)
{
   Efl2_Canvas_Text_Data *o = efl_data_scope_get(cur->obj, EFL2_CANVAS_TEXT_CLASS);
   Evas_Object_Textblock_Node_Text *n;
   size_t npos = 0;
...
void
_canvas_text_cursor_position_set(Efl2_Text_Cursor_Handle *cur, int _pos)
{
   Efl2_Canvas_Text_Data *o = efl_data_scope_get(cur->obj, EFL2_CANVAS_TEXT_CLASS);
   Evas_Object_Textblock_Node_Text *n;
   size_t pos;
...
Eina_Bool
_canvas_text_cursor_line_number_set(Efl2_Text_Cursor_Handle *cur, int line)
{
   Evas_Object_Textblock_Line *ln;
...
_canvas_text_cursor_line_jump_by(Efl2_Text_Cursor_Handle *cur, int by)
{
   ...
   ln = _canvas_text_cursor_line_number_get(cur) + by;
...
int
_canvas_text_cursor_compare(const Efl2_Text_Cursor_Handle *cur1, const Efl2_Text_Cursor_Handle *cur2)
{
   Eina_Inlist *l1, *l2;
...
Eina_Bool
_canvas_text_cursor_equal(const Efl2_Text_Cursor_Handle *cur, const Efl2_Text_Cursor_Handle *cur2)
{
   return ((cur->node == cur2->node) && (cur->pos == cur2->pos));
}

void
_canvas_text_cursor_copy(Efl2_Text_Cursor_Handle *dst, const Efl2_Text_Cursor_Handle *src)
{
   if (src->obj != dst->obj)
     {
 ...
 */
static void
_evas_textblock_cursor_break_paragraph(Efl2_Text_Cursor_Handle *cur,
                              Evas_Object_Textblock_Node_Format *fnode,
                              Eina_Bool legacy)
{
   Evas_Object_Textblock_Node_Text *n;
...
{
   Eina_List *l;
   Efl2_Text_Cursor_Handle *cur = o->cursor;
   Efl2_Text_Cursor_Handle *data;
...
static inline void
_canvas_text_cursor_update_offset(Efl2_Text_Cursor_Handle *cur, Efl2_Canvas_Text_Data *o,
 ...
 */
static void
_evas_textblock_cursors_update_offset(const Efl2_Text_Cursor_Handle *cur,
      const Evas_Object_Textblock_Node_Text *n,
...
_evas_textblock_changed(Efl2_Canvas_Text_Data *o, Evas_Object *eo_obj)
{
   Evas_Object_Protected_Data *obj = efl_data_scope_get(eo_obj, EFL_CANVAS_OBJECT_CLASS);
   Eina_List *l;
   Efl2_Text_Cursor_Handle *data_obj;
...
static Eina_Bool
_canvas_text_cursor_is_at_the_end(const Efl2_Text_Cursor_Handle *cur)
{
   const Eina_Unicode *text;
...
static Eina_Bool
_evas_textblock_cursor_format_append(Efl2_Text_Cursor_Handle *cur,
      ...
      Eina_Bool is_item)
{
   ...
   if (!o->text_nodes)
     {
        _canvas_text_cursor_text_insert(cur, "");
     }
   ...
   Efl2_Text_Cursor_Handle *ocur = o->cursor;
   if (!ocur->node)
...
void
_canvas_text_cursor_char_delete(Efl2_Text_Cursor_Handle *cur)
{
...
void
_canvas_text_cursor_range_delete(Efl2_Text_Cursor_Handle *cur1, Efl2_Text_Cursor_Handle *cur2)
{
   Evas_Object_Textblock_Node_Format *fnode = NULL;
   ...
   if (_canvas_text_cursor_compare(cur1, cur2) > 0)
     {
        Efl2_Text_Cursor_Handle *tc;
   ...
   n1 = cur1->node;
   n2 = cur2->node;
   if ((_canvas_text_cursor_compare(o->cursor, cur1) >= 0) &&
         (_canvas_text_cursor_compare(cur2, o->cursor) >= 0))
     {
   ...
   if (reset_cursor)
     _canvas_text_cursor_copy(cur1, o->cursor);

   _evas_textblock_changed(o, cur1->obj);
   efl_event_callback_call(eo_obj, EFL2_CANVAS_TEXT_EVENT_CHANGED, NULL);
}
...
char *
_canvas_text_cursor_text_plain_get(const Efl2_Text_Cursor_Handle *cur1, const Efl2_Text_Cursor_Handle *_cur2)
{
   Eina_UStrbuf *buf;
   Evas_Object_Textblock_Node_Text *n1, *n2;
   Efl2_Text_Cursor_Handle *cur2;
   ...
   if (_canvas_text_cursor_compare(cur1, _cur2) > 0)
     {
        const Efl2_Text_Cursor_Handle *tc;
   ...
   cur2 = alloca(sizeof(Efl2_Text_Cursor_Handle));
   cur2->obj = _cur2->obj;
   _canvas_text_cursor_copy(cur2, _cur2);
...
void
_evas_textblock_cursor_at_format_set(Efl2_Text_Cursor_Handle *cur,
...
static Eina_Bool
_evas_textblock_cursor_format_is_visible_get(const Efl2_Text_Cursor_Handle *cur)
{
   const Eina_Unicode *text;
...
int
_canvas_text_cursor_line_number_get(const Efl2_Text_Cursor_Handle *cur)
{
   Evas_Object_Textblock_Line *ln = NULL;
   Evas_Object_Textblock_Item *it = NULL;
   int x, y, w, h;
...
_evas_textblock_cursor_range_in_line_geometry_get(
      const Evas_Object_Textblock_Line *ln, const Efl2_Text_Cursor_Handle *cur1,
      const Efl2_Text_Cursor_Handle *cur2)
{
   ...
   const Efl2_Text_Cursor_Handle *cur;
...
Eina_Iterator *
_canvas_text_cursor_range_simple_geometry_get(const Efl2_Text_Cursor_Handle *cur1, const Efl2_Text_Cursor_Handle *cur2)
{
   ...
   itr = _canvas_text_selection_iterator_new(rects);

   return itr;
}

Eina_List *
_canvas_text_cursor_range_precise_geometry_get(const Efl2_Text_Cursor_Handle *cur1, const Efl2_Text_Cursor_Handle *cur2)
{
   Evas_Object_Textblock_Line *ln1, *ln2;
   Evas_Object_Textblock_Item *it1, *it2;
   Eina_List *rects = NULL;
...
static Eina_Bool
_evas_textblock_cursor_format_item_geometry_get(const Efl2_Text_Cursor_Handle *cur, Evas_Coord *cx, Evas_Coord *cy, Evas_Coord *cw, Evas_Coord *ch)
{
   Evas_Object_Textblock_Line *ln = NULL;
   Evas_Object_Textblock_Format_Item *fi;
   Evas_Object_Textblock_Item *it = NULL;
...
_efl2_canvas_text_size_formatted_get(const Eo *eo_obj, Efl2_Canvas_Text_Data *o)
{
   Evas_Object_Protected_Data *obj = efl_data_scope_get(eo_obj, EFL_CANVAS_OBJECT_CLASS);
   evas_object_async_block(obj);
   _relayout_if_needed(eo_obj, o);
...
EOLIAN static Eina_Size2D
_efl2_canvas_text_size_native_get(const Eo *eo_obj, Efl2_Canvas_Text_Data *o)
{
   Evas_Object_Protected_Data *obj = efl_data_scope_get(eo_obj, EFL_CANVAS_OBJECT_CLASS);
   evas_object_async_block(obj);
   if (!o->native.valid)
...
_efl2_canvas_text_efl_object_dbg_info_get(Eo *eo_obj, Efl2_Canvas_Text_Data *o EINA_UNUSED, Efl_Dbg_Info *root)
{
   ...
   EFL_DBG_INFO_APPEND(group, "Text", EINA_VALUE_TYPE_STRING, shorttext);
     ...
     {
        Eina_Size2D size = efl2_canvas_text_size_formatted_get(eo_obj);
     ...
     {
        Eina_Size2D size = efl2_canvas_text_size_native_get(eo_obj);
   ...
   while (o->cursors)
     {
        ...
        cur = (Efl2_Text_Cursor_Handle *)o->cursors->data;
...
{
   Evas_Object_Protected_Data *obj = efl_data_scope_get(eo_obj, EFL_CANVAS_OBJECT_CLASS);
   Efl2_Canvas_Text_Data *o = efl_data_scope_get(eo_obj, EFL2_CANVAS_TEXT_CLASS);
...
{
   Efl2_Text_Cursor_Handle start, end;
...
static Eina_Bool
_textblock_annotation_insert_internal(Eo *eo_obj EINA_UNUSED, Efl2_Canvas_Text_Data *o,
      Efl2_Text_Cursor_Handle *start, Efl2_Text_Cursor_Handle *end,
      Efl2_Text_Attribute_Handle *an)
{
   int len;
   char *buf;
   Evas_Textblock_Node_Format *fnode;
   const char *tag = an->tag;
   const char *format = an->properties;
   Eina_Bool is_item = an->is_item;

   /* Add opening format at 'start' */
   len = strlen(tag) + strlen(format);
   buf = malloc(len + 4);
   sprintf(buf, "<%s %s>", tag, format);
...
void
_canvas_text_annotation_free_if_not_reffed(Efl2_Text_Attribute_Handle *an)
{
   if (!an->obj && (an->ref == 0))
     {
        free(an);
...
_evas_textblock_annotation_remove(Efl2_Canvas_Text_Data *o,
      Efl2_Text_Attribute_Handle *an, Eina_Bool remove_nodes)
{
   if (remove_nodes)
     {
        if (an->is_item)
   ...
   o->annotations = (Efl2_Text_Attribute_Handle *)
      eina_inlist_remove(EINA_INLIST_GET(o->annotations),
            EINA_INLIST_GET(an));
   an->obj = NULL;
   _canvas_text_annotation_free_if_not_reffed(an);
}
...
Eina_Bool
_canvas_text_annotation_remove(Eo *eo_obj, Efl2_Text_Attribute_Handle *annotation)
{
   Efl2_Canvas_Text_Data *o = efl_data_scope_get(eo_obj, MY_CLASS);
   ASYNC_BLOCK;
...
Efl2_Text_Attribute_Handle *
_textblock_annotation_insert(Efl2_Text_Cursor_Handle *start, Efl2_Text_Cursor_Handle *end,
      Efl2_Text_Attribute_Handle *ret)
{
   Eo *eo_obj = start->obj;
   Efl2_Canvas_Text_Data *o = efl_data_scope_get(eo_obj, MY_CLASS);
   ...
   const char *item;
   const char *tag = ret->tag;
   const char *format = ret->properties;

   if (!format || (format[0] == '\0') ||
         _canvas_text_cursor_compare(start, end) > 0)
     {
        return NULL;
     }
   ...
   if (format && (format[0] != '\0'))
     {
        ...
        Eina_Bool is_item = !strcmp(tag, "item");
        ret->is_item = is_item;
        _textblock_annotation_insert_internal(eo_obj, o, start, end, ret);
        _evas_textblock_changed(o, eo_obj);
     }
...
Eina_Iterator *
_canvas_text_range_annotations_get(const Efl2_Text_Cursor_Handle *start, const Efl2_Text_Cursor_Handle *end)
{
   Efl2_Canvas_Text *eo_obj = start->obj;
   Efl2_Canvas_Text_Data *o = efl_data_scope_get(eo_obj, MY_CLASS);
   Eina_List *lst = NULL;
   Efl2_Text_Attribute_Handle *it;

   EINA_INLIST_FOREACH(o->annotations, it)
     {
        Efl2_Text_Cursor_Handle start2, end2;
        _canvas_text_cursor_init(&start2, eo_obj);
        _canvas_text_cursor_init(&end2, eo_obj);
...
_text_layout_async_done(void *todo, Ecore_Thread *thread EINA_UNUSED)
{
   ...
   if (o->layout_jobs > 0)
     {
        efl2_canvas_text_async_layout(obj);
     }
}
