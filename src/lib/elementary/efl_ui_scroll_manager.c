...
_inside_range(int value, int min, int max)
{
  if (value < min) return EINA_FALSE;
  if (value > max) return EINA_FALSE;
  return EINA_TRUE;
}
...
EOLIAN static void
_efl_ui_scroll_manager_mirrored_set(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd, Eina_Bool mirrored)
{
   Evas_Coord wx;
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollbar_mode_set(Eo *obj EINA_UNUSED,
                                                 Efl_Ui_Scroll_Manager_Data *sd,
                                                 ...
                                                 Efl_Ui_Scrollbar_Mode vmode)
{
   ...
   efl_ui_scrollbar_visibility_update(sd->obj);
}

EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollbar_mode_get(Eo *obj EINA_UNUSED,
                                                 Efl_Ui_Scroll_Manager_Data *sd,
                                                 Efl_Ui_Scrollbar_Mode *hmode,
                                                 Efl_Ui_Scrollbar_Mode *vmode)
{
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollable_interactive_show(Eo *obj, Efl_Ui_Scroll_Manager_Data *sd, Eina_Rect rect)
{
...
_scroll_wheel_post_event_go(Efl_Ui_Scroll_Manager_Data *sd, int x, int y)
{
   ...
   if (_elm_config->scroll_animation_disable)
     {
        efl_future_then(efl_loop_job(efl_loop_get(sd->obj), NULL),
                        _scroll_wheel_post_event_job, NULL, NULL, sd);
     }
   else
     {
        _scroll_manager_scrollto(sd, x, y);
     }
}
...
static Eina_Bool
_efl_ui_scroll_manager_post_event_up(void *data,
                          Evas *e EINA_UNUSED)
{
   Efl_Ui_Scroll_Manager_Data *sd = data;
...
_efl_ui_scroll_manager_scroll_to_x_animator(void *data, const Efl_Event *event EINA_UNUSED)
{
   ...
   _efl_ui_scroll_manager_wanted_coordinates_update(sd, nx, cy);

   if (dt >= sd->scrollto.x.dur)
   {
...
_efl_ui_scroll_manager_scroll_to_y_animator(void *data, const Efl_Event *event EINA_UNUSED)
{
   ...
   _efl_ui_scroll_manager_wanted_coordinates_update(sd, cx, ny);

   if (dt >= sd->scrollto.y.dur)
   {
                              ...
                              void *event_info)
{
   ...
   ev = event_info;
   evas_post_event_callback_push(e, _efl_ui_scroll_manager_post_event_up, sd);
...
static void
_efl_ui_scroll_manager_pan_viewport_changed_cb(void *data, const Efl_Event *event)
{
   Efl_Ui_Scroll_Manager_Data *sd = data;
...
static void
_efl_ui_scroll_manager_pan_position_changed_cb(void *data, const Efl_Event *event)
{
   Efl_Ui_Scroll_Manager_Data *sd = data;
...
Eina_Bool _hbar_hide_timer(void *data)
{
   Efl_Ui_Scroll_Manager_Data *sd = data;
...
Eina_Bool _vbar_hide_timer(void *data)
{
   Efl_Ui_Scroll_Manager_Data *sd = data;
...
#define EFL_UI_SCROLLBAR_TIMER 1.0
static void
   ...
   if (sd->hbar_visible)
     {
        ...
        if (sd->hbar_mode == EFL_UI_SCROLLBAR_MODE_HIDDEN)
          {
             ELM_SAFE_FREE(sd->hbar_timer, ecore_timer_del);
             sd->hbar_timer = ecore_timer_add(EFL_UI_SCROLLBAR_TIMER, _hbar_hide_timer, sd);
          }
     }
   else
     {
   ...
   if (sd->vbar_visible)
     {
        Efl_Ui_Scrollbar_Direction type = EFL_UI_SCROLLBAR_DIRECTION_VERTICAL;
        efl_event_callback_call(sd->parent, EFL_UI_SCROLLBAR_EVENT_BAR_SHOW, &type);
        if (sd->vbar_mode == EFL_UI_SCROLLBAR_MODE_HIDDEN)
          {
             ELM_SAFE_FREE(sd->vbar_timer, ecore_timer_del);
             sd->vbar_timer = ecore_timer_add(EFL_UI_SCROLLBAR_TIMER, _vbar_hide_timer, sd);
          }
     }
   else
     {
   ...
   switch (sd->hbar_mode)
     {
	  ...
	  case EFL_UI_SCROLLBAR_MODE_HIDDEN:
         if (sd->hbar_visible)
           {
              if (w <= view.w)
                {
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollbar_visibility_update(Eo *obj, Efl_Ui_Scroll_Manager_Data *sd EINA_UNUSED)
{
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollbar_position_set(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd, double posx, double posy)
{
   Evas_Coord x, y;
   Eina_Position2D min = {0, 0}, max = {0, 0};
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollbar_position_get(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd, double *posx, double *posy)
{
   if (!sd->pan_obj) return;
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollbar_size_get(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd, double *width, double *height)
{
   if (!sd->pan_obj) return;
...
_efl_ui_scroll_manager_efl_ui_scrollable_interactive_hold_get(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd)
{
   return sd->hold;
}

EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollable_interactive_hold_set(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd, Eina_Bool hold)
{
   sd->hold = hold;
}

EOLIAN static Eina_Bool
_efl_ui_scroll_manager_efl_ui_scrollable_interactive_freeze_get(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd)
{
   return sd->freeze;
}

EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollable_interactive_freeze_set(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd, Eina_Bool freeze)
{
   sd->freeze = freeze;
   if (sd->freeze)
     {
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollable_interactive_scroll(Eo *obj, Efl_Ui_Scroll_Manager_Data *sd, Eina_Rect rect)
{
   _scroll_manager_animators_drop(obj);
   if (_efl_ui_scroll_manager_content_region_show_internal(obj, &(rect.x), &(rect.y), rect.w, rect.h))
     {
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollable_interactive_loop_set(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd, Eina_Bool loop_h, Eina_Bool loop_v)
{
...
EOLIAN static void
_efl_ui_scroll_manager_efl_ui_scrollable_interactive_loop_get(Eo *obj EINA_UNUSED, Efl_Ui_Scroll_Manager_Data *sd, Eina_Bool *loop_h, Eina_Bool *loop_v)
{
   *loop_h = sd->loop_h;
   *loop_v = sd->loop_v;
}
