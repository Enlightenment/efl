...
//	       
  for (arg=1; arg<argc; arg++) {
    ...
    if (isoption) {
      ...
      #if !defined SC_LIGHT
        parserespf(&argv[arg][1],iname,oname,ename,pname,rname);
      #endif
  ...
  strcpy(pname,sDEF_PREFIX);
    ...
    if (argv[0]!=NULL) {
      ...
      if (access(cfgfile,4)==0)
        parserespf(cfgfile,iname,oname,ename,pname,rname);
    } /* if */
  #endif
  parseoptions(argc,argv,iname,oname,ename,pname,rname);
  if (strlen(iname)==0)
    ...
    if (numdim==1) {
      *size=initvector(ident,tag,dim[0],FALSE);
      ...
      for (d=0; d<dim[0]; d++) {
        ...
        if (d+1<dim[0])
          needtoken(',');
        if (matchtoken('{') || matchtoken(tSTRING))  /* expect a '{' or a string */
          lexpush();
        else
          break;
      } /* for */
