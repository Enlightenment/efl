   ...
   for (int i = 0; i < ECORE_EVAS_SELECTION_BUFFER_LAST; ++i)
     {
        if (data->callbacks[i].cancel)
          data->callbacks[i].cancel(ee, i);
     }
}

Eina_Bool
fallback_selection_claim(Ecore_Evas *ee, unsigned int seat, Ecore_Evas_Selection_Buffer selection, Eina_Array *available_types, Eina_Bool (*delivery)(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer buffer, const char *type, Eina_Rw_Slice *slice), void (*cancel)(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer buffer))
{
   Ecore_Evas_Selection_Callbacks *callbacks = &data->callbacks[selection];

   if (callbacks->cancel)
     {
        callbacks->cancel(ee, selection);
...
Eina_Future*
fallback_selection_request(Ecore_Evas *ee EINA_UNUSED, unsigned int seat EINA_UNUSED, Ecore_Evas_Selection_Buffer selection, Eina_Array *acceptable_type)
{
   Ecore_Evas_Selection_Callbacks callbacks = data->callbacks[selection];
   Eina_Content *result;
   ...
   EINA_SAFETY_ON_FALSE_RETURN_VAL(callbacks.delivery(ee, selection, serving_type, &slice_data), NULL);
...
Eina_Bool
fallback_dnd_start(Ecore_Evas *ee EINA_UNUSED, unsigned int seat EINA_UNUSED, Eina_Array *available_types EINA_UNUSED, Ecore_Evas *drag_rep EINA_UNUSED, Eina_Bool (*delivery)(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer buffer, const char *type, Eina_Rw_Slice *slice) EINA_UNUSED, void (*cancel)(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer buffer) EINA_UNUSED, const char* action EINA_UNUSED)
{
   return EINA_FALSE;
}
