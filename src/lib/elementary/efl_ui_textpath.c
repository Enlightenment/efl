...
{
   edje_object_part_text_style_user_pop(pd->text_obj, "text");

   if (enabled)
     edje_object_part_text_style_user_push(pd->text_obj, "text",
...
_ellipsis_set(Efl_Ui_Textpath_Data *pd)
{
   ...
   tb = edje_object_part_object_get(pd->text_obj, "text");
...
{
   _textpath_text_set_internal(obj, pd, "text", text);
}

EOLIAN static const char *
_efl_ui_textpath_efl_text_text_get(const Eo *obj EINA_UNUSED, Efl_Ui_Textpath_Data *pd)
{
   return edje_object_part_text_get(pd->text_obj, "text");
}
