...
struct _Ecore_Evas_Engine_Data_X11 {
   ...
   Ecore_Evas_X11_Selection_Data seleciton_data[ECORE_EVAS_SELECTION_BUFFER_LAST];
...
{
   Ecore_Evas_Engine_Data_X11 *edata = ee->engine.data;
   Ecore_Evas_Selection_Callbacks *cbs = &edata->seleciton_data[selection].callbacks;
...
{
   Ecore_Evas_Engine_Data_X11 *edata = ee->engine.data;
   eina_stringshare_replace(&edata->seleciton_data[selection].requested_type, NULL);
   eina_stringshare_replace(&edata->seleciton_data[selection].later_conversion, NULL);
   edata->seleciton_data[selection].delivery = NULL;
   eina_array_free(edata->seleciton_data[selection].acceptable_type);
   edata->seleciton_data[selection].acceptable_type = NULL;
}
...
static void
_search_fitting_type(Eina_Array *arr, Ecore_Evas_Selection_Buffer selection, Ecore_Evas_Engine_Data_X11 *edata, Ecore_Evas *ee)
{
...
#define HANDLE_TYPE() \
   { \
      edata->seleciton_data[selection].requested_type = eina_stringshare_add(x11_name); \
      edata->seleciton_data[selection].later_conversion = eina_stringshare_add(acceptable_type);\
      found_conversion = EINA_TRUE; \
      break; \
   }

   EINA_SAFETY_ON_NULL_RETURN(edata->seleciton_data[selection].acceptable_type);
   ...
   for (unsigned int i = 0; i < eina_array_count(arr) && !found_conversion; ++i)
     {
        ...
        for (unsigned int j = 0; j < eina_array_count(edata->seleciton_data[selection].acceptable_type) && !found_conversion; ++j)
          {
             const char *acceptable_type = (const char*) eina_array_data_get(edata->seleciton_data[selection].acceptable_type, j);
   ...
   if (found_conversion)
     {
        _ecore_x_selection_request(ee->prop.window, selection, edata->seleciton_data[selection].requested_type);
     }
   else
     {
        eina_promise_resolve(edata->seleciton_data[selection].delivery, eina_value_error_init(ecore_evas_no_matching_type));
...
static void
_search_fitting_type_from_event(Ecore_X_Event_Selection_Notify *ev, Ecore_Evas_Selection_Buffer selection, Ecore_Evas_Engine_Data_X11 *edata, Ecore_Evas *ee)
{
   ...
   _search_fitting_type(tmp, selection, edata, ee);
   eina_array_free(tmp);
}

static void
_deliver_content(Ecore_X_Event_Selection_Notify *ev, Ecore_Evas_Selection_Buffer selection, Ecore_Evas_Engine_Data_X11 *edata, Ecore_Evas *ee)
{
   Ecore_X_Selection_Data *x11_data = ev->data;
   Eina_Rw_Slice data;
   Eina_Content *result;
   Eina_Stringshare *mime_type = _decrypt_type(edata->seleciton_data[selection].requested_type);
   ...
   if (edata->seleciton_data[selection].later_conversion != mime_type)
     {
        Eina_Content *tmp = eina_content_convert(result, edata->seleciton_data[selection].later_conversion);
        eina_content_free(result);
        result = tmp;
     }

   eina_promise_resolve(edata->seleciton_data[selection].delivery, eina_value_content_init(result));
   eina_content_free(result);
   _clear_selection_delivery(ee, selection);
...
_ecore_evas_x_selection_notify(void *udata EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   if (selection == ECORE_EVAS_SELECTION_BUFFER_DRAG_AND_DROP_BUFFER &&
       !edata->seleciton_data[selection].later_conversion)
     {
        ecore_x_dnd_send_finished();
     }
   else
        ...
        if (eina_streq(ev->target, "TARGETS") || eina_streq(ev->target, "ATOMS"))
          {
             ...
             EINA_SAFETY_ON_FALSE_RETURN_VAL(!edata->seleciton_data[selection].later_conversion, EINA_FALSE);
             EINA_SAFETY_ON_FALSE_RETURN_VAL(!edata->seleciton_data[selection].requested_type, EINA_FALSE);
             _search_fitting_type_from_event(ev, selection, edata, ee);
          }
        else
          {
             ...
             EINA_SAFETY_ON_FALSE_RETURN_VAL(edata->seleciton_data[selection].later_conversion, EINA_FALSE);
             EINA_SAFETY_ON_FALSE_RETURN_VAL(edata->seleciton_data[selection].requested_type, EINA_FALSE);
             _deliver_content(ev, selection, edata, ee);
          }
     }
end:
   return ECORE_CALLBACK_PASS_ON;
...
_ecore_evas_x_selection_clear(void *udata EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   EINA_SAFETY_ON_FALSE_GOTO(selection != ECORE_EVAS_SELECTION_BUFFER_LAST, end);
   edata = ee->engine.data;
   cbs = &edata->seleciton_data[selection].callbacks;
...
_force_stop_self_dnd(Ecore_Evas *ee)
{
   ...
   if (ee->drag.free)
     ee->drag.free(ee, ee->drag.data, ee->drag.accepted);
   ee->drag.free = NULL;
}
        ...
        for (unsigned int i = 0; i < eina_array_count(sdata->callbacks.available_types); ++i)
          {
             if (mime_type == eina_array_data_get(sdata->callbacks.available_types, i))
               {
                  Eina_Rw_Slice slice;
                  sdata->callbacks.delivery(sdata->ee, sdata->buffer, mime_type, &slice);
                  *size_ret = slice.len;
                  *data_ret = slice.mem;
...
_ecore_evas_x_dnd_enter(void *udata EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   ecore_evas_dnd_enter(ee, 0, eina_array_iterator_new(mime_tmp), EINA_POSITION2D(0,0)); //FIXME
...
_ecore_evas_x_dnd_leave(void *udata EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   EINA_SAFETY_ON_NULL_GOTO(ee, end);
   edata = ee->engine.data;
   ecore_evas_dnd_leave(ee, 0, EINA_POSITION2D(0,0));
...
_ecore_evas_x_dnd_position(void *udata EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   ecore_evas_dnd_position_set(ee, 0, EINA_POSITION2D(pos->position.x - x, pos->position.y - y));
...
_ecore_evas_x_dnd_drop(void *udata EINA_UNUSED, int type EINA_UNUSED, void *event)
{
   ...
   if (ee->func.fn_dnd_drop)
     ee->func.fn_dnd_drop(ee, 0, ecore_evas_dnd_pos_get(ee, 0), _x11_dnd_action_map(drop->action));
   if (!edata->seleciton_data[ECORE_EVAS_SELECTION_BUFFER_DRAG_AND_DROP_BUFFER].requested_type)
     {
        ecore_x_dnd_send_finished();
     }
   ecore_evas_dnd_leave(ee, 0, EINA_POSITION2D(drop->position.x ,drop->position.y));
   eina_array_free(edata->xserver_atom_name_during_dnd);
   edata->xserver_atom_name_during_dnd = NULL;
end:
   return ECORE_CALLBACK_PASS_ON;
...
static Eina_Bool
_ecore_evas_x_selection_has_owner(Ecore_Evas *ee EINA_UNUSED, unsigned int seat EINA_UNUSED, Ecore_Evas_Selection_Buffer selection EINA_UNUSED)
{
...
_deliver_selection_changed(void *data)
{
   ...
   if (_ecore_evas_x_selection_has_owner(ee, 0, ECORE_EVAS_SELECTION_BUFFER_SELECTION_BUFFER))
     ee->func.fn_selection_changed(ee, 0, ECORE_EVAS_SELECTION_BUFFER_SELECTION_BUFFER);
   if (_ecore_evas_x_selection_has_owner(ee, 0, ECORE_EVAS_SELECTION_BUFFER_COPY_AND_PASTE_BUFFER))
     ee->func.fn_selection_changed(ee, 0, ECORE_EVAS_SELECTION_BUFFER_COPY_AND_PASTE_BUFFER);
   if (_ecore_evas_x_selection_has_owner(ee, 0, ECORE_EVAS_SELECTION_BUFFER_DRAG_AND_DROP_BUFFER))
     ee->func.fn_selection_changed(ee, 0, ECORE_EVAS_SELECTION_BUFFER_DRAG_AND_DROP_BUFFER);
}
   ...
   for (int i = 0; i < ECORE_EVAS_SELECTION_BUFFER_LAST; ++i)
     {
        ...
        edata->seleciton_data[i].ee = ee;
        edata->seleciton_data[i].buffer = i;
     }
   ecore_x_dnd_aware_set(ee->prop.window, EINA_TRUE);
   ecore_job_add(_deliver_selection_changed, ee);
}
...
_store_selection_cbs(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer selection, Eina_Array *available_types, Eina_Bool (*delivery)(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer buffer, const char *type, Eina_Rw_Slice *slice), void (*cancel)(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer buffer))
{
   ...
   edata = ee->engine.data;
   sdata = &edata->seleciton_data[selection];
   cbs = &sdata->callbacks;
   ...
   if (selection == ECORE_EVAS_SELECTION_BUFFER_DRAG_AND_DROP_BUFFER)
     {
        ...
        EINA_SAFETY_ON_FALSE_RETURN_VAL(!edata->seleciton_data[selection].later_conversion, NULL);
        EINA_SAFETY_ON_FALSE_RETURN_VAL(!edata->seleciton_data[selection].requested_type, NULL);
        _search_fitting_type(edata->xserver_atom_name_during_dnd, selection, edata, ee);
     }
   else
     {
...
static Eina_Bool
_ecore_evas_x_dnd_start(Ecore_Evas *ee, unsigned int seat EINA_UNUSED, Eina_Array *available_types, Ecore_Evas *drag_rep, Eina_Bool (*delivery)(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer buffer, const char *type, Eina_Rw_Slice *slice), void (*cancel)(Ecore_Evas *ee, Ecore_Evas_Selection_Buffer buffer), const char* action)
{
   Ecore_Evas_X11_Selection_Data *sdata;
   Ecore_Evas_Engine_Data_X11 *edata;
   Ecore_X_Atom actx;
   ...
   edata = ee->engine.data;
   sdata = &edata->seleciton_data[ECORE_EVAS_SELECTION_BUFFER_DRAG_AND_DROP_BUFFER];
