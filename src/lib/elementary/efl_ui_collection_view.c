...
_entity_propagate(Efl_Model *model, Efl_Gfx_Entity *entity)
{
   ...
   if (ITEM_SIZE_FROM_MODEL(model, item_size))
     {
        /* efl_event_freeze(entity); */
        efl_key_data_set(entity, "efl.ui.widget.factory.size_set", (void*)EINA_TRUE);
...
static void
_mark_greater(Efl_Ui_Collection_Item_Lookup *root, Eina_Array *mark, const unsigned int upper)
{
   if (!root) return ;
...
_idle_cb(void *data, const Efl_Event *event EINA_UNUSED)
{
   ...
   _mark_greater((void*) pd->cache, &mark, upper_end);
...
_cache_cleanup_above(Efl_Ui_Collection_View *obj, Efl_Ui_Collection_View_Data *pd, unsigned int index)
{
   ...
   _mark_greater((void*) pd->cache, &mark, index);
...
_efl_model_child_removed(void *data, const Efl_Event *event)
{
   ...
   _cache_cleanup_above(data, pd, ev->index - 1);
