...
START_TEST(eo_cxx_future_construct_and_destroy)
{
...

   {
      ...
      efl::shared_future<int> future(eo_ref(f));
   }
   ecore_shutdown();
}
END_TEST
...
START_TEST(eo_cxx_future_then_value)
{
   ...
   promise = eo_add(EFL_PROMISE_CLASS, ecore_main_loop_get());
   fail_if(!promise);
   ...
   {
      efl::shared_future<int> future(eo_ref(f)), rfuture;
...
START_TEST(eo_cxx_future_composite_construct_and_destroy)
{
...

   {
      ...
      efl::shared_future<int> future1(eo_ref(f1))
        , future2(eo_ref(f2));
      efl::shared_future<int, int> future3(eo_ref(f3));
   }
   ecore_shutdown();
}
END_TEST
