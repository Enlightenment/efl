...
static void
_op_mul_pas_dp_mmx(DATA32 *s, DATA8 *m, DATA32 c, DATA32 *d, int l) {
   DATA32 *e = s + l;
   pxor_r2r(mm0, mm0);
   MOV_P2R(PIX_WHITE, mm6, mm0)
   while (s < e)
...
init_mul_pixel_span_funcs_mmx(void)
{
   ...
   op_mul_span_funcs[SP][SM_N][SC_N][DP_AS][CPU_MMX] = _op_mul_p_dpas_mmx;
   op_mul_span_funcs[SP_AN][SM_N][SC_N][DP_AS][CPU_MMX] = _op_mul_pan_dpas_mmx;
   op_mul_span_funcs[SP_AS][SM_N][SC_N][DP_AS][CPU_MMX] = _op_mul_pas_dpas_mmx;
}
#endif
...
#define _op_mul_pt_p_dpas_mmx _op_mul_pt_p_dp_mmx
#define _op_mul_pt_pan_dpas_mmx _op_mul_pt_pan_dp_mmx
#define _op_mul_pt_pas_dpas_mmx _op_mul_pt_pas_dp_mmx
...
init_mul_pixel_pt_funcs_mmx(void)
{
   ...
   op_mul_pt_funcs[SP][SM_N][SC_N][DP_AS][CPU_MMX] = _op_mul_pt_p_dpas_mmx;
   op_mul_pt_funcs[SP_AN][SM_N][SC_N][DP_AS][CPU_MMX] = _op_mul_pt_pan_dpas_mmx;
   op_mul_pt_funcs[SP_AS][SM_N][SC_N][DP_AS][CPU_MMX] = _op_mul_pt_pas_dpas_mmx;
}
#endif
