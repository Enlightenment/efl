...
_dbus_session_removed(DBusMessage *msg)
{
   ...
   if (!strcmp(n, _ecore_input_session_id))
...
_dbus_setup(void)
{
   ...
   ret = asprintf(&dpath, "/org/freedesktop/login1/session/%s", 
...
void 
_ecore_input_dbus_shutdown(void)
{
#ifdef HAVE_LIBINPUT
...
int 
_ecore_input_dbus_device_open(const char *device)
{
#ifdef HAVE_LIBINPUT
...
void 
_ecore_input_dbus_device_close(int fd)
{
#ifdef HAVE_LIBINPUT
   struct stat st;
   int ret;

   if ((ret = fstat(fd, &st)) < 0) return;
