...
_deliverty_cb(void *data, const Eina_Value value, const Eina_Future *dead_future EINA_UNUSED)
{
   ...
   if (!content) return EINA_VALUE_EMPTY;
...
static void
_efl_ui_terminated(Ecore_Evas *ee EINA_UNUSED, void *data, Eina_Bool accepted EINA_UNUSED)
{
   efl_del(data);
}
...
efl_main(void *data EINA_UNUSED, const Efl_Event *ev EINA_UNUSED)
{
   ...
   Eina_Accessor *acc = efl_core_command_line_command_access(efl_main_loop_get());
   char *goal;
   ...
   EINA_SAFETY_ON_FALSE_RETURN(eina_accessor_data_get(acc, 1, (void**)&goal));
   ...
   if (eina_streq(goal, "--monitor"))
   ...
   else if (eina_streq(goal, "--set-selection"))
     {
        ...
        if (!eina_accessor_data_get(acc, 2, (void**)&selection))
          {
