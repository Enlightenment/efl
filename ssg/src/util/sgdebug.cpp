...
namespace
{
    ...
    typedef std::tuple < char, uint32_t, uint64_t, int32_t, int64_t, double, SGDebug::string_literal_t, char * > SupportedTypes;
    ...
    *reinterpret_cast<Arg*>(buffer()) = arg;
    ...
    template < typename Arg >
    void SGDebug::encode(Arg arg, uint8_t type_id)
    {
    ...
    SGDebug::SGDebug(LogLevel level, char const * file, char const * function, uint32_t line)
    : m_bytes_used(0)
    ...
    void SGDebug::stringify(std::ostream & os)
    {
    ...
    template <>
    char * decode(std::ostream & os, char * b, SGDebug::string_literal_t * dummy)
    {
    SGDebug::string_literal_t s = *reinterpret_cast < SGDebug::string_literal_t * >(b);
    os << s.m_s;
    return b + sizeof(SGDebug::string_literal_t);
    }
    ...
    void SGDebug::stringify(std::ostream & os, char * start, char const * const end)
    {
    if (start == end)
        return;
    ...
    void SGDebug::resize_buffer_if_needed(size_t additional_bytes)
    {
    ...
    void SGDebug::encode(char const * arg)
    {
    if (arg != nullptr)
        encode_c_string(arg, strlen(arg));
    }

    void SGDebug::encode(char * arg)
    {
    if (arg != nullptr)
        encode_c_string(arg, strlen(arg));
    }

    void SGDebug::encode_c_string(char const * arg, size_t length)
    {
    if (length == 0)
        return;
...
//    }
    ...
    virtual ~BufferBase() = default;
        virtual void push(SGDebug && logline) = 0;
    virtual bool try_pop(SGDebug & logline) = 0;
    };
    ...
    public:
        ...
        char written;
        char padding[256 - sizeof(std::atomic_flag) - sizeof(char) - sizeof(SGDebug)];
        SGDebug logline;
        };
        ...
        void push(SGDebug && logline) override
        {
        ...
        bool try_pop(SGDebug & logline) override
        {
    ...
    public:
        struct Item
        {
        Item(SGDebug && logline) : logline(std::move(logline)) {}
        char padding[256 - sizeof(SGDebug)];
        SGDebug logline;
        };
    ...
    // Returns true if we need to switch to next buffer
        bool push(SGDebug && logline, unsigned int const write_index)
        {
        ...
        bool try_pop(SGDebug & logline, unsigned int const read_index)
        {
    ...
    void pop()
    {
        ...
        SGDebug logline(LogLevel::INFO, nullptr, nullptr, 0);
    ...
    bool SGDebugServer::operator==(SGDebug & logline)
    {
