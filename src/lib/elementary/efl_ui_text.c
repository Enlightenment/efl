...
_validate(Evas_Object *obj)
{
   ...
   vc.text = edje_object_part_text_get(sd->entry_edje, "text");
...
_edje_entry_user_insert(Evas_Object *obj, const char *data)
{
   ...
   edje_object_part_text_user_insert(sd->entry_edje, "text", data);
            ...
            Elm_Xdnd_Action action EINA_UNUSED)
{
   ...
   edje_object_part_text_cursor_coord_set
      (sd->entry_edje, "text", EDJE_CURSOR_USER, x, y);
   pos = edje_object_part_text_cursor_pos_get
      (sd->entry_edje, "text", EDJE_CURSOR_USER);
   edje_object_part_text_cursor_pos_set(sd->entry_edje, "text",
              ...
              Elm_Selection_Data *drop)
{
   ...
   rv = edje_object_part_text_cursor_coord_set
       (sd->entry_edje, "text", EDJE_CURSOR_MAIN, drop->x, drop->y);
...
_efl_ui_text_background_switch(Evas_Object *from_edje, Evas_Object *to_edje)
{
   ...
   if (edje_object_part_exists(from_edje, "background") &&
       edje_object_part_exists(to_edje, "background") &&
       !edje_object_part_swallow_get(to_edje, "background"))
     {
        bg_obj = edje_object_part_swallow_get(from_edje, "background");

        if (bg_obj)
          {
             edje_object_part_unswallow(from_edje, bg_obj);
             edje_object_part_swallow(to_edje, "background", bg_obj);
          }
     }
}
...
_efl_ui_text_efl_ui_widget_theme_apply(Eo *obj, Efl_Ui_Text_Data *sd)
{
   ...
   edje_object_part_text_input_panel_layout_set
     (sd->entry_edje, "text", (Edje_Input_Panel_Layout)sd->input_panel_layout);
   edje_object_part_text_input_panel_layout_variation_set
     (sd->entry_edje, "text", sd->input_panel_layout_variation);
   edje_object_part_text_autocapital_type_set
     (sd->entry_edje, "text", (Edje_Text_Autocapital_Type)sd->autocapital_type);
   edje_object_part_text_prediction_allow_set
     (sd->entry_edje, "text", sd->prediction_allow);
   edje_object_part_text_input_hint_set
     (sd->entry_edje, "text", (Edje_Input_Hints)sd->input_hints);
   edje_object_part_text_input_panel_enabled_set
     (sd->entry_edje, "text", sd->input_panel_enable);
   edje_object_part_text_input_panel_imdata_set
     (sd->entry_edje, "text", sd->input_panel_imdata,
     sd->input_panel_imdata_len);
   edje_object_part_text_input_panel_return_key_type_set
     (sd->entry_edje, "text", (Edje_Input_Panel_Return_Key_Type)sd->input_panel_return_key_type);
   ...
   edje_object_part_text_input_panel_show_on_demand_set
     (sd->entry_edje, "text", sd->input_panel_show_on_demand);
...
_cursor_geometry_recalc(Evas_Object *obj)
{
   ...
   evas_object_geometry_get(
         edje_object_part_swallow_get(sd->entry_edje, "text"),
         &x2, &y2, &w2, &h2);
   cx = cx + x - x2;
   cy = cy + y - y2;
...
_layout_text_sizing_eval(Eo *obj, Evas_Coord tw, Evas_Coord th)
{
   ...
   sw = edje_object_part_swallow_get(sd->entry_edje, "text");
   if (sd->scroll)
     {
...
typedef struct _Layout_Ctx Layout_Ctx;
struct _Layout_Ctx
{
   Eo *obj;
};
...
static Eina_Value
_on_layout_complete(void *data, const Eina_Value v)
{
   Layout_Ctx *c = data;
   Eina_Rectangle r;
   ...
   if (can_async)
     {
        ...
        c->obj = obj;
        efl_event_thaw(obj);
        f = efl_canvas_text_async_layout(sw);
        eina_future_then_easy(f, _on_layout_complete, NULL, NULL, NULL, c);
     }
   else
     {
...
_efl_ui_text_efl_ui_focus_object_on_focus_update(Eo *obj, Efl_Ui_Text_Data *sd)
{
   ...
   if (efl_ui_focus_object_focus_get(obj))
     {
        Eo *sw = edje_object_part_swallow_get(sd->entry_edje, "text");
             ...
             if ((sd->have_selection) && (!sd->hoversel))
               {
                  ...
                  edje_object_part_text_select_allow_set(sd->entry_edje, "text", EINA_FALSE);
                  edje_object_signal_emit(sd->entry_edje, "elm,state,select,off", "elm");
                  edje_object_part_text_select_none(sd->entry_edje, "text");
               }
          }
...
_efl_ui_text_efl_ui_widget_widget_sub_object_del(Eo *obj, Efl_Ui_Text_Data *_pd EINA_UNUSED, Evas_Object *sobj)
{
    ...
    * smart function */
   if (sobj == elm_layout_content_get(obj, "icon"))
     {
        elm_layout_signal_emit(obj, "elm,action,hide,icon", "elm");
     }
   else if (sobj == elm_layout_content_get(obj, "end"))
     {
...
_hoversel_position(Evas_Object *obj)
{
   ...
   if (sd->use_down)
     ...
     edje_object_part_text_cursor_geometry_get
       (sd->entry_edje, "text", &cx, &cy, &cw, &ch);
        ...
        if (!_elm_config->desktop_entry)
          {
             edje_object_part_text_select_allow_set
               (sd->entry_edje, "text", EINA_FALSE);
             edje_object_part_text_select_abort(sd->entry_edje, "text");
          }
     }
}
...
_magnifier_create(void *data)
{
   ...
   evas_object_event_callback_add(sd->mgf_proxy, EVAS_CALLBACK_MOVE,
                                  _magnifier_proxy_update, data);
   edje_object_part_swallow(sd->mgf_bg, "content", sd->mgf_proxy);
...
_magnifier_move(void *data)
{
   ...
   edje_object_part_text_cursor_geometry_get(sd->entry_edje, "text",
                                             &cx, &cy, NULL, &ch);
   if (sd->scroll)
     {
        Evas_Coord ox, oy;
             ...
             if (rv)
               {
                  edje_object_part_text_cursor_copy
                    (sd->entry_edje, "text", EDJE_CURSOR_USER, EDJE_CURSOR_MAIN);
               }
             else
   ...
   if (sd->single_line != single_line)
     {
        ...
        efl_ui_widget_theme_apply(data);
     }
                                ...
                                const char *source EINA_UNUSED)
{
   ...
   sd->cursor_pos = edje_object_part_text_cursor_pos_get
       (sd->entry_edje, "text", EDJE_CURSOR_MAIN);
                                        ...
                                        Elm_Entry_Anchor_Info *ei)
{
   ...
   geoms = edje_object_part_text_anchor_geometry_get
       (sd->entry_edje, "text", ei->name);

   if (!geoms) return;

   evas_object_geometry_get(
      edje_object_part_object_get(sd->entry_edje, "text"),
      &x, &y, NULL, NULL);
                             ...
                             void *event_info)
{
   ...
   Eo *text_obj = edje_object_part_swallow_get(sd->entry_edje, "text");
...
_efl_ui_text_efl_object_constructor(Eo *obj, Efl_Ui_Text_Data *sd)
{
   ...
   edje_object_part_swallow(sd->entry_edje, "text", text_obj);
   ...
   edje_object_text_markup_filter_callback_add
     (sd->entry_edje, "text", _markup_filter_cb, obj);
   ...
   edje_object_signal_callback_add
     (sd->entry_edje, "entry,changed,user", "text",
     _entry_changed_user_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "preedit,changed", "text",
     _entry_preedit_changed_signal_cb, obj);

   edje_object_signal_callback_add
     (sd->entry_edje, "cursor,changed", "text",
     _entry_cursor_changed_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "cursor,changed,manual", "text",
     _entry_cursor_changed_manual_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "anchor,mouse,down,*", "text",
     _entry_anchor_down_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "anchor,mouse,up,*", "text",
     _entry_anchor_up_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "anchor,mouse,move,*", "text",
     _entry_anchor_move_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "anchor,mouse,in,*", "text",
     _entry_anchor_in_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "anchor,mouse,out,*", "text",
     _entry_anchor_out_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "entry,key,enter", "text",
     _entry_key_enter_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "entry,key,escape", "text",
     _entry_key_escape_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "mouse,down,1", "text",
     _entry_mouse_down_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "mouse,clicked,1", "text",
     _entry_mouse_clicked_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "mouse,down,1,double", "text",
     _entry_mouse_double_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "mouse,down,1,triple", "text",
     _entry_mouse_triple_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "entry,undo,request", "text",
     _entry_undo_request_signal_cb, obj);
   edje_object_signal_callback_add
     (sd->entry_edje, "entry,redo,request", "text",
...
_efl_ui_text_efl_object_destructor(Eo *obj, Efl_Ui_Text_Data *sd)
{
   ...
   text_obj = edje_object_part_swallow_get(sd->entry_edje, "text");
...
{
   edje_object_part_text_select_extend(sd->entry_edje, "text");
}
...
_efl_ui_text_input_panel_layout_set(Eo *obj EINA_UNUSED, Efl_Ui_Text_Data *sd, Elm_Input_Panel_Layout layout)
{
   ...
   edje_object_part_text_input_panel_layout_set
     (sd->entry_edje, "text", (Edje_Input_Panel_Layout)layout);
...
_efl_ui_text_autocapital_type_set(Eo *obj EINA_UNUSED, Efl_Ui_Text_Data *sd, Elm_Autocapital_Type autocapital_type)
{
   ...
   edje_object_part_text_autocapital_type_set
     (sd->entry_edje, "text", (Edje_Text_Autocapital_Type)autocapital_type);
}
...
_efl_ui_text_input_hint_set(Eo *obj EINA_UNUSED, Efl_Ui_Text_Data *sd, Elm_Input_Hints hints)
{
   ...
   edje_object_part_text_input_hint_set
     (sd->entry_edje, "text", (Edje_Input_Hints)hints);
}
...
{
   edje_object_part_text_input_panel_show(sd->entry_edje, "text");
}
...
_efl_ui_text_input_panel_hide(Eo *obj EINA_UNUSED, Efl_Ui_Text_Data *sd)
{
   ...
   edje_object_part_text_input_panel_hide(sd->entry_edje, "text");
}
...
_efl_ui_text_input_panel_return_key_type_set(Eo *obj EINA_UNUSED, Efl_Ui_Text_Data *sd, Elm_Input_Panel_Return_Key_Type return_key_type)
{
   ...
   edje_object_part_text_input_panel_return_key_type_set
     (sd->entry_edje, "text", (Edje_Input_Panel_Return_Key_Type)return_key_type);
}
...
_efl_ui_text_efl_access_text_string_get(const Eo *obj EINA_UNUSED, Efl_Ui_Text_Data *pd, Efl_Access_Text_Granularity granularity, int *start_offset, int *end_offset)
{
   ...
   char *ret = NULL;
   Eo *text_obj = edje_object_part_swallow_get(pd->entry_edje, "text");
...
_efl_ui_text_efl_access_i18n_name_get(const Eo *obj, Efl_Ui_Text_Data *pd)
{
   ...
   const char *ret = edje_object_part_text_get(pd->entry_edje, "guide");
   return ret;
}
...
_efl_ui_text_selection_changed_cb(void *data, const Efl_Event *event EINA_UNUSED)
{
   ...
   if (!text || (text[0] == '\0'))
     {
        _edje_signal_emit(sd, "selection,cleared", "text");
        sd->have_selection = EINA_FALSE;
     }
   else
     {
        if (!sd->have_selection)
          {
             _edje_signal_emit(sd, "selection,start", "text");
          }
        _edje_signal_emit(sd, "selection,changed", "text");
