...
ecore_evas_new_internal_common(const char *disp_name, int rotation, int w, int h, const char *engine_type)
{
   Evas_Engine_Info_FB *einfo;
   ...
   rmethod = evas_render_method_lookup(engine_type);
...
{
   ecore_evas_new_internal_common(disp_name, rotation, w, h, "fb");
}
...
{
   ecore_evas_new_internal_common(disp_name, rotation, w, h, "eglfs");
}
#endif
