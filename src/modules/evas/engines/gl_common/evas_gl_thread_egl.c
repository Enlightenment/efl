...
typedef struct
{
   GL_TH_FNTYPE(eglBindWaylandDisplay) orig_func;
   EGLBoolean return_value;
   EGLDisplay dpy;
   void *display;
} GL_TH_ST(eglBindWaylandDisplay);

static void
GL_TH_CB(eglBindWaylandDisplay)(void *data)
{
   GL_TH_ST(eglBindWaylandDisplay) *thread_data = *(void **)data;
...
EGLBoolean
GL_TH_FN(eglBindWaylandDisplay)(GL_TH_DP, EGLDisplay dpy, void *display)
{
   GL_TH_ST(eglBindWaylandDisplay) thread_data_local, *thread_data = &thread_data_local, **thread_data_ptr;
   void *thcmd_ref;

   if (!evas_gl_thread_enabled(thread_type))
     return ((GL_TH_FNTYPE(eglBindWaylandDisplay))orig_func)(dpy, display);

   thread_data_ptr =
      evas_gl_thread_cmd_create(thread_type, sizeof(GL_TH_ST(eglBindWaylandDisplay) *), &thcmd_ref);
   *thread_data_ptr = thread_data;
   ...
   evas_gl_thread_cmd_enqueue(thcmd_ref,
                              GL_TH_CB(eglBindWaylandDisplay),
                              EVAS_GL_THREAD_MODE_FINISH);
...
typedef struct
{
   GL_TH_FNTYPE(eglUnbindWaylandDisplay) orig_func;
   EGLBoolean return_value;
   EGLDisplay dpy;
   void *display;
} GL_TH_ST(eglUnbindWaylandDisplay);

static void
GL_TH_CB(eglUnbindWaylandDisplay)(void *data)
{
   GL_TH_ST(eglUnbindWaylandDisplay) *thread_data = *(void **)data;
...
EGLBoolean
GL_TH_FN(eglUnbindWaylandDisplay)(GL_TH_DP, EGLDisplay dpy, void *display)
{
   GL_TH_ST(eglUnbindWaylandDisplay) thread_data_local, *thread_data = &thread_data_local, **thread_data_ptr;
   void *thcmd_ref;

   if (!evas_gl_thread_enabled(thread_type))
     return ((GL_TH_FNTYPE(eglUnbindWaylandDisplay))orig_func)(dpy, display);

   thread_data_ptr =
      evas_gl_thread_cmd_create(thread_type, sizeof(GL_TH_ST(eglUnbindWaylandDisplay) *), &thcmd_ref);
   *thread_data_ptr = thread_data;
   ...
   evas_gl_thread_cmd_enqueue(thcmd_ref,
                              GL_TH_CB(eglUnbindWaylandDisplay),
                              EVAS_GL_THREAD_MODE_FINISH);
...
typedef struct
{
   GL_TH_FNTYPE(eglCreateSync) orig_func;
   EvasGLSync return_value;
   EGLDisplay dpy;
   EGLenum type;
   const EGLint *attrib_list;
} GL_TH_ST(eglCreateSync);

static void
GL_TH_CB(eglCreateSync)(void *data)
{
   GL_TH_ST(eglCreateSync) *thread_data = *(void **)data;
...
EvasGLSync
GL_TH_FN(eglCreateSync)(GL_TH_DP, EGLDisplay dpy, EGLenum type, const EGLint *attrib_list)
{
   GL_TH_ST(eglCreateSync) thread_data_local, *thread_data = &thread_data_local, **thread_data_ptr;
   void *thcmd_ref;

   if (!evas_gl_thread_enabled(thread_type))
     return ((GL_TH_FNTYPE(eglCreateSync))orig_func)(dpy, type, attrib_list);

   thread_data_ptr =
      evas_gl_thread_cmd_create(thread_type, sizeof(GL_TH_ST(eglCreateSync) *), &thcmd_ref);
   *thread_data_ptr = thread_data;
...
typedef struct
{
   GL_TH_FNTYPE(eglDestroySync) orig_func;
   EGLBoolean return_value;
   EGLDisplay dpy;
   EvasGLSync sync;
} GL_TH_ST(eglDestroySync);

static void
GL_TH_CB(eglDestroySync)(void *data)
{
   GL_TH_ST(eglDestroySync) *thread_data = *(void **)data;
...
EGLBoolean
GL_TH_FN(eglDestroySync)(GL_TH_DP, EGLDisplay dpy, EvasGLSync sync)
{
   GL_TH_ST(eglDestroySync) thread_data_local, *thread_data = &thread_data_local, **thread_data_ptr;
   void *thcmd_ref;

   if (!evas_gl_thread_enabled(thread_type))
     return ((GL_TH_FNTYPE(eglDestroySync))orig_func)(dpy, sync);

   thread_data_ptr =
      evas_gl_thread_cmd_create(thread_type, sizeof(GL_TH_ST(eglDestroySync) *), &thcmd_ref);
   *thread_data_ptr = thread_data;
...
typedef struct
{
   GL_TH_FNTYPE(eglClientWaitSync) orig_func;
   EGLint return_value;
   EGLDisplay dpy;
   EvasGLSync sync;
   EGLint flags;
   EvasGLTime timeout;
} GL_TH_ST(eglClientWaitSync);

static void
GL_TH_CB(eglClientWaitSync)(void *data)
{
   GL_TH_ST(eglClientWaitSync) *thread_data = *(void **)data;
...
EGLint
GL_TH_FN(eglClientWaitSync)(GL_TH_DP, EGLDisplay dpy, EvasGLSync sync, EGLint flags, EvasGLTime timeout)
{
   GL_TH_ST(eglClientWaitSync) thread_data_local, *thread_data = &thread_data_local, **thread_data_ptr;
   void *thcmd_ref;

   if (!evas_gl_thread_enabled(thread_type))
     return ((GL_TH_FNTYPE(eglClientWaitSync))orig_func)(dpy, sync, flags, timeout);

   thread_data_ptr =
      evas_gl_thread_cmd_create(thread_type, sizeof(GL_TH_ST(eglClientWaitSync) *), &thcmd_ref);
   *thread_data_ptr = thread_data;
...
typedef struct
{
   GL_TH_FNTYPE(eglSignalSync) orig_func;
   EGLBoolean return_value;
   EGLDisplay dpy;
   EvasGLSync sync;
   EGLenum mode;
} GL_TH_ST(eglSignalSync);

static void
GL_TH_CB(eglSignalSync)(void *data)
{
   GL_TH_ST(eglSignalSync) *thread_data = *(void **)data;
...
EGLBoolean
GL_TH_FN(eglSignalSync)(GL_TH_DP, EGLDisplay dpy, EvasGLSync sync, EGLenum mode)
{
   GL_TH_ST(eglSignalSync) thread_data_local, *thread_data = &thread_data_local, **thread_data_ptr;
   void *thcmd_ref;

   if (!evas_gl_thread_enabled(thread_type))
     return ((GL_TH_FNTYPE(eglSignalSync))orig_func)(dpy, sync, mode);

   thread_data_ptr =
      evas_gl_thread_cmd_create(thread_type, sizeof(GL_TH_ST(eglSignalSync) *), &thcmd_ref);
   *thread_data_ptr = thread_data;
...
typedef struct
{
   GL_TH_FNTYPE(eglGetSyncAttrib) orig_func;
   EGLBoolean return_value;
   EGLDisplay dpy;
   EvasGLSync sync;
   EGLint attribute;
   EGLint *value;
} GL_TH_ST(eglGetSyncAttrib);

static void
GL_TH_CB(eglGetSyncAttrib)(void *data)
{
   GL_TH_ST(eglGetSyncAttrib) *thread_data = *(void **)data;
...
EGLBoolean
GL_TH_FN(eglGetSyncAttrib)(GL_TH_DP, EGLDisplay dpy, EvasGLSync sync, EGLint attribute, EGLint *value)
{
   GL_TH_ST(eglGetSyncAttrib) thread_data_local, *thread_data = &thread_data_local, **thread_data_ptr;
   void *thcmd_ref;

   if (!evas_gl_thread_enabled(thread_type))
     return ((GL_TH_FNTYPE(eglGetSyncAttrib))orig_func)(dpy, sync, attribute, value);

   thread_data_ptr =
      evas_gl_thread_cmd_create(thread_type, sizeof(GL_TH_ST(eglGetSyncAttrib) *), &thcmd_ref);
   *thread_data_ptr = thread_data;
...
typedef struct
{
   GL_TH_FNTYPE(eglWaitSync) orig_func;
   EGLint return_value;
   EGLDisplay dpy;
   EvasGLSync sync;
   EGLint flags;
} GL_TH_ST(eglWaitSync);

static void
GL_TH_CB(eglWaitSync)(void *data)
{
   GL_TH_ST(eglWaitSync) *thread_data = *(void **)data;
...
EGLint
GL_TH_FN(eglWaitSync)(GL_TH_DP, EGLDisplay dpy, EvasGLSync sync, EGLint flags)
{
   GL_TH_ST(eglWaitSync) thread_data_local, *thread_data = &thread_data_local, **thread_data_ptr;
   void *thcmd_ref;

   if (!evas_gl_thread_enabled(thread_type))
     return ((GL_TH_FNTYPE(eglWaitSync))orig_func)(dpy, sync, flags);

   thread_data_ptr =
      evas_gl_thread_cmd_create(thread_type, sizeof(GL_TH_ST(eglWaitSync) *), &thcmd_ref);
   *thread_data_ptr = thread_data;
