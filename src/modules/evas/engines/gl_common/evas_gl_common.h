...
#   define GL_ERROR_TRACE(thf, f, _args, ...) do { GLERRV("Uncaught error before this point:"); GL_TH_CALL(thf, f, ##__VA_ARGS__); GLERRV(#f); } while(0)
...
#  define glGetUniformLocation(...) GL_ERROR_TRACE_RET(GLuint, glGetUniformLocation, glGetUniformLocation, #__VA_ARGS__, __VA_ARGS__)
...
#  define glMapBuffer(...) GL_ERROR_TRACE_RET(void *, glMapBuffer, glMapBuffer, #__VA_ARGS__, __VA_ARGS__)
...
#  define glCreateShader(...) GL_ERROR_TRACE_RET(GLuint, glCreateShader, glCreateShader, #__VA_ARGS__, __VA_ARGS__)
#  define glCreateProgram(...) GL_ERROR_TRACE_RET(GLuint, glCreateProgram, glCreateProgram, #__VA_ARGS__, __VA_ARGS__)
...
#  define eglsym_eglDestroyImage(...) GL_ERROR_TRACE_RET(unsigned int, eglDestroyImage, eglsym_eglDestroyImage, #__VA_ARGS__, __VA_ARGS__)
