...
/* Thread Environment */
static int env_evas_gl_no_render_thread = -1;
...
evas_gl_thread_enabled(int thread_type)
{
   ...
   if (UNLIKELY(env_evas_gl_no_render_thread == 1))
      return 0;
...
#define BUILD_MEMPOOL(name) \
   ...
   char *env_thread = getenv("EVAS_GL_RENDER_THREAD_OFF");
   int env_thread_value = 0;
   if (env_thread)
      env_thread_value = atoi(env_thread);

   if (env_thread_value == 1)
      env_evas_gl_no_render_thread = 1;
   else
      env_evas_gl_no_render_thread = 0;
