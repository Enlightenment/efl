                      ...
                      unsigned int type, const int *attrib_list)
{
   ...
   if (!dpy) return NULL;
   return EVGL_TH_CALL(eglCreateSync, EXT_FUNC_EGL(eglCreateSyncKHR), dpy, type, attrib_list);
}
...
_evgl_evasglDestroySync(Evas_GL *evas_gl, EvasGLSync sync)
{
   ...
   return EVGL_TH_CALL(eglDestroySync, EXT_FUNC_EGL(eglDestroySyncKHR), dpy, sync);
}
                          ...
                          EvasGLSync sync, int flags, EvasGLTime timeout)
{
   EGLDisplay dpy = EGLDISPLAY_GET(evas_gl);
   if (!dpy) return EINA_FALSE;
   return EVGL_TH_CALL(eglClientWaitSync, EXT_FUNC_EGL(eglClientWaitSyncKHR), dpy, sync, flags, timeout);
}
                      ...
                      EvasGLSync sync, unsigned mode)
{
   EGLDisplay dpy = EGLDISPLAY_GET(evas_gl);
   if (!dpy) return EINA_FALSE;
   return EVGL_TH_CALL(eglSignalSync, EXT_FUNC_EGL(eglSignalSyncKHR), dpy, sync, mode);
}
                         ...
                         EvasGLSync sync, int attribute, int *value)
{
   EGLDisplay dpy = EGLDISPLAY_GET(evas_gl);
   if (!dpy) return EINA_FALSE;
   return EVGL_TH_CALL(eglGetSyncAttrib, EXT_FUNC_EGL(eglGetSyncAttribKHR), dpy, sync, attribute, value);
}
                    ...
                    EvasGLSync sync, int flags)
{
   EGLDisplay dpy = EGLDISPLAY_GET(evas_gl);
   if (!dpy) return EINA_FALSE;
   return EVGL_TH_CALL(eglWaitSync, EXT_FUNC_EGL(eglWaitSyncKHR), dpy, sync, flags);
}
                              ...
                              void *wl_display)
{
   EGLDisplay dpy = EGLDISPLAY_GET(evas_gl);
   if (!dpy) return EINA_FALSE;
   return EVGL_TH_CALL(eglBindWaylandDisplay, EXT_FUNC_EGL(eglBindWaylandDisplayWL), dpy, wl_display);
}
                                ...
                                void *wl_display)
{
   EGLDisplay dpy = EGLDISPLAY_GET(evas_gl);
   if (!dpy) return EINA_FALSE;
   return EVGL_TH_CALL(eglUnbindWaylandDisplay, EXT_FUNC_EGL(eglUnbindWaylandDisplayWL), dpy, wl_display);
}
                              ...
                              void *buffer, int attribute, int *value)
{
   EGLDisplay dpy = EGLDISPLAY_GET(evas_gl);
   if (!dpy) return EINA_FALSE;
   return EVGL_TH_CALL(eglQueryWaylandBuffer, EXT_FUNC_EGL(eglQueryWaylandBufferWL), dpy, buffer, attribute, value);
}
