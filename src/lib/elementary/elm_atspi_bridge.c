...
typedef struct _Elm_Atspi_Bridge_Data
{
   ...
   struct {
        Eldbus_Service_Interface *cache;
...
static void _state_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _bounds_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _property_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _children_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _window_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _visible_data_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _active_descendant_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _selection_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _text_text_inserted_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _text_text_removed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _text_caret_moved_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _text_selection_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _object_added_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
static void _object_removed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event);
...
static Eina_Bool _bridge_object_registered_is(Eo *bridge, Eo *obj);
static void _bridge_address_from_object(Eo *bridge, const Eo *eo, struct dbus_address *address);
...
static void _bridge_disconnect(Elm_Atspi_Bridge *bridge);
static void _bridge_connect(Elm_Atspi_Bridge *bridge);
static void _bridge_enabled_set(Elm_Atspi_Bridge *bridge, Eina_Bool value);
static void _bridge_on_disconnected(Elm_Atspi_Bridge *bridge);
static void _bridge_on_connected(Elm_Atspi_Bridge *bridge);
static void _bridge_pending_del(Elm_Atspi_Bridge *bridge, Eldbus_Pending *pending);
static void _bridge_pending_add(Elm_Atspi_Bridge *obj, Eldbus_Pending *pending, Eldbus_Free_Cb cb, const void *data);
static void _bridge_pending_cancel_all(Elm_Atspi_Bridge *bridge);
static void _bridge_plug_register(Eo *obj, Elm_Atspi_Plug *plug);
static void _bridge_plug_unregister(Eo *obj, Elm_Atspi_Plug *plug);
static void _bridge_object_added_signal_send(Elm_Atspi_Bridge *bridge, Elm_Interface_Atspi_Accessible *obj);
static void _bridge_object_removed_signal_send(Elm_Atspi_Bridge *bridge, Elm_Interface_Atspi_Accessible *obj);
...
static void _socket_hooks_uninstall(Elm_Atspi_Socket *socket);
static void _plug_hooks_install(Elm_Atspi_Plug *plug);
static void _plug_hooks_uninstall(Elm_Atspi_Plug *plug);


enum _Atspi_Object_Child_Event_Type
{
   ATSPI_OBJECT_ACCESSIBLE_ADDED = 0,
   ATSPI_OBJECT_ACCESSIBLE_REMOVED
};
...
static const Eldbus_Method socket_methods[] = {
   { "Embedded", ELDBUS_ARGS({"s", "parentObject"}), NULL, _socket_embedded, 0 },
   { NULL, NULL, NULL, NULL, 0 }
};
...
static void
_bridge_address_from_object(Eo *bridge, const Eo *eo, struct dbus_address *addr)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);

   if (!eo)
     {
        snprintf(addr->bus, sizeof(addr->bus), "%s", eldbus_connection_unique_name_get(pd->a11y_bus));
        snprintf(addr->path, sizeof(addr->path), "%s", ATSPI_DBUS_PATH_NULL);
     }
   else if (eo == pd->root)
     {
        snprintf(addr->bus, sizeof(addr->bus), "%s", eldbus_connection_unique_name_get(pd->a11y_bus));
        snprintf(addr->path, sizeof(addr->path), "%s%s", ELM_ACCESS_OBJECT_PATH_PREFIX, ELM_ACCESS_OBJECT_PATH_ROOT);
     }
   else if (efl_isa(eo, ELM_ATSPI_PROXY_CLASS))
     {
        if (!_id_parse(elm_atspi_proxy_id_get(eo), addr))
          {
     ...
     {
        snprintf(addr->bus, sizeof(addr->bus), "%s", eldbus_connection_unique_name_get(pd->a11y_bus));
        snprintf(addr->path, sizeof(addr->path), ELM_ACCESS_OBJECT_REFERENCE_TEMPLATE, (unsigned long long)(uintptr_t)eo);
     }
}
   ...
   while (to_process)
     {
        ...
        // do not return cache info for non-local objects
        if (efl_isa(obj, ELM_ATSPI_PROXY_CLASS)) continue;

        _cache_item_reference_append_cb(bridge, obj, iter_array);

        Eina_List *children = elm_interface_atspi_accessible_children_get(obj);
...
static const Eldbus_Signal cache_signals[] = {
  [ATSPI_OBJECT_ACCESSIBLE_ADDED] = { "AddAccessible", ELDBUS_ARGS({"((so)(so)a(so)assusau)", "added"}), 0},
  [ATSPI_OBJECT_ACCESSIBLE_REMOVED] = { "RemoveAccessible", ELDBUS_ARGS({ "(so)", "removed" }), 0},
  {NULL, NULL, 0}
};
...
static void
_on_plug_register(void *data, const Eldbus_Message *msg, Eldbus_Pending *pending)
{
   ...
   if (eldbus_message_error_get(msg, &errname, &errmsg))
     {
        _bridge_pending_del(data, pending);
...
static void
_bridge_plugs_register(Eo *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
   assert (pd->connected);
   Eina_List *l;
...
EAPI void
_bridge_plugs_unregister(Eo *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
   assert (!pd->connected);
   Eina_List *l;
        ...
        if (!tokens[1] || *tokens[1] == '\0') return; // do not handle "Object:*"
        ...
        else if (!strcmp(tokens[1], "ChildrenChanged"))
          {
             if (!tokens[2] || *tokens[2] == '\0')
               ...
               STATE_TYPE_SET(pd->object_children_broadcast_mask, ATSPI_OBJECT_ACCESSIBLE_ADDED);
             else if (!strcmp(tokens[2], "remove"))
               STATE_TYPE_SET(pd->object_children_broadcast_mask, ATSPI_OBJECT_ACCESSIBLE_REMOVED);
          }
...
_registered_events_list_update(Eo *bridge)
{
   ...
   _bridge_pending_add(bridge, p, NULL, NULL);
}
...
static void
_state_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
...
static void
_bounds_changed_event_handle(Elm_Atspi_Bridge *bridge, const Elm_Accessible_Event *event)
{
   Elm_Atspi_Event_Geometry_Changed_Data *geo_data = event->info;

   _bridge_signal_send(bridge, event->object, ATSPI_DBUS_INTERFACE_EVENT_OBJECT,
                       &_event_obj_signals[ATSPI_OBJECT_EVENT_BOUNDS_CHANGED], "", 0, 0, "(iiii)",
                       geo_data->x, geo_data->y, geo_data->width, geo_data->height);
}

static void
_property_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
...
static void
_visible_data_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(data, pd);
...
static void
_active_descendant_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   Eo *child = event->info;
   int idx;
...
static void
_children_changed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   ...
   if (ev_data->is_added)
     {
        type = ATSPI_OBJECT_ACCESSIBLE_ADDED;
        atspi_desc = "add";
        _bridge_object_register(data, ev_data->child);
     }
   else
     {
        type = ATSPI_OBJECT_ACCESSIBLE_REMOVED;
        atspi_desc = "remove";
...
static void
_window_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
...
static void
_selection_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(data, pd);
...
static void _bridge_signal_send(Eo *bridge, Eo *obj, const char *infc, const Eldbus_Signal *signal, const char *minor, unsigned int det1, unsigned int det2, const char *variant_sig, ...)
{
             ...
             switch (*tmp)
               {
                ...
                case 'o':
                   ...
                   _bridge_address_from_object(bridge, atspi_obj, &addr);;
   ...
   iter_struct = eldbus_message_iter_container_new(iter, 'r', NULL);
   _bridge_address_from_object(bridge, pd->root, &addr);
   eldbus_message_iter_basic_append(iter_struct, 's', addr.bus);
   eldbus_message_iter_basic_append(iter_struct, 'o', addr.path);
   eldbus_message_iter_container_close(iter, iter_struct);
...
static void
_text_caret_moved_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   int cursor_pos = 0;
...
static void
_text_text_inserted_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
...
static void
_text_text_removed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   Elm_Atspi_Text_Change_Info *info = event->info;
...
static void
_text_selection_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(data, pd);
...
static void
_bridge_event_handlers_register(Eo *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
static void
_bridge_on_object_unregistered(Elm_Atspi_Bridge *bridge EINA_UNUSED, Eo *obj)
{
   ...
   if (efl_isa(obj, ELM_ATSPI_PLUG_CLASS))
     {
        ...
        _plug_hooks_uninstall(obj);
     }
   if (efl_isa(obj, ELM_ATSPI_SOCKET_CLASS))
     {
        _socket_hooks_uninstall(obj);
     }
}

static void
_bridge_on_object_registered(Elm_Atspi_Bridge *bridge, Eo *obj)
{
   struct dbus_address addr;
   ...
   if (efl_isa(obj, ELM_ATSPI_PLUG_CLASS))
     {
        _plug_hooks_install(obj);
        _bridge_address_from_object(bridge, obj, &addr);
        char *id = _id_make(&addr);
   ...
   if (efl_isa(obj, ELM_ATSPI_SOCKET_CLASS))
     {
        _socket_hooks_install(obj);
     }
}

static Eina_Bool
_bridge_object_registered_is(Eo *bridge, Eo *obj)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN_VAL(bridge, pd, EINA_FALSE);

   if (pd->connected && eina_hash_find(pd->cache, &obj))
     return EINA_TRUE;
   else
     return EINA_FALSE;
}
...
_bridge_object_unregister(Eo *bridge, Eo *obj)
{
   ...
   // unregister object and all its descendants
   Eina_List *to_process = eina_list_append(NULL, obj);
   while (to_process)
     {
        Eo *o = eina_list_data_get(to_process);
        to_process = eina_list_remove_list(to_process, to_process);

        if (!eina_hash_find(pd->cache, &o))
             continue;

        eina_hash_del(pd->cache, &o, o);

        _bridge_on_object_unregistered(bridge, o);

        Eina_List *children = elm_interface_atspi_accessible_children_get(o);
        to_process = eina_list_merge(to_process, children);
     }
}

static void
_bridge_object_added_signal_send(Elm_Atspi_Bridge *bridge, Elm_Interface_Atspi_Accessible *object)
{
   Eldbus_Message *sig;
   Eldbus_Message_Iter *iter;
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);

   sig = eldbus_service_signal_new(pd->interfaces.cache, ATSPI_OBJECT_ACCESSIBLE_ADDED);
   iter = eldbus_message_iter_get(sig);
   _cache_item_reference_append_cb(bridge, object, iter);

   eldbus_service_signal_send(pd->interfaces.cache, sig);
}

static void
_object_added_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   _bridge_object_added_signal_send(data, event->object);
}

static void
_bridge_object_removed_signal_send(Elm_Atspi_Bridge *bridge, Elm_Interface_Atspi_Accessible *object)
{
   Eldbus_Message *sig;
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);

   sig = eldbus_service_signal_new(pd->interfaces.cache, ATSPI_OBJECT_ACCESSIBLE_REMOVED);
   Eldbus_Message_Iter *iter = eldbus_message_iter_get(sig);
   _bridge_iter_object_reference_append(bridge, iter, object);
   eldbus_service_signal_send(pd->interfaces.cache, sig);
}

static void
_object_removed_event_handle(Elm_Atspi_Bridge *data, const Elm_Accessible_Event *event)
{
   _bridge_object_removed_signal_send(data, event->object);
}

static void
_bridge_interfaces_unregister(Eo *bridge)
{
    ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
#define INTERFACE_SAFE_FREE(ifc) \
   ...
   INTERFACE_SAFE_FREE(pd->interfaces.cache);
#undef INTERFACE_SAFE_FREE
}

static void
_bridge_interfaces_register(Eo *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);

   pd->interfaces.cache = eldbus_service_interface_register(pd->a11y_bus, CACHE_INTERFACE_PATH, &cache_iface_desc);
   eldbus_service_object_data_set(pd->interfaces.cache, ELM_ATSPI_BRIDGE_CLASS_NAME, bridge);
...
EOLIAN static void
_elm_atspi_bridge_elm_interface_accessible_observer_on_event(Elm_Atspi_Bridge *bridge, Elm_Atspi_Bridge_Data *pd EINA_UNUSED, const Elm_Accessible_Event *event)
{
   // register plug object if created
   if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_ADDED && efl_isa(event->object, ELM_ATSPI_PLUG_CLASS))
     _bridge_plug_register(bridge, event->object);

   // unregister plug object if deleted
   if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_REMOVED && efl_isa(event->object, ELM_ATSPI_PLUG_CLASS))
     _bridge_plug_unregister(bridge, event->object);

   // broadcast only singals from objects registered in cache
   if (!_bridge_object_registered_is(bridge, event->object))
       return;

   // handle event
   if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_CHILDREN_CHANGED)
     _children_changed_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_PROPERTY_CHANGED)
     _property_changed_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_BOUNDS_CHANGED)
     _bounds_changed_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_STATE_CHANGED)
      _state_changed_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_VISIBLE_DATA_CHANGED)
      _visible_data_changed_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_ACTIVE_DESCENDANT_CHANGED)
      _active_descendant_changed_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_ADDED)
      _object_added_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_ACCESSIBLE_EVENT_REMOVED)
      _object_removed_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_WINDOW_EVENT_WINDOW_CREATED)
      _window_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_WINDOW_EVENT_WINDOW_DESTROYED)
      _window_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_WINDOW_EVENT_WINDOW_ACTIVATED)
      _window_event_handle(bridge, event);
      ...
      _window_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_SELECTION_EVENT_SELECTION_CHANGED)
      _selection_event_handle(bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_TEXT_EVENT_ACCESS_TEXT_CARET_MOVED)
      _text_caret_moved_event_handle (bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_TEXT_EVENT_ACCESS_TEXT_INSERTED)
      _text_text_inserted_event_handle (bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_TEXT_EVENT_ACCESS_TEXT_REMOVED)
      _text_text_removed_event_handle (bridge, event);
   else if (event->desc == ELM_INTERFACE_ATSPI_TEXT_EVENT_ACCESS_TEXT_SELECTION_CHANGED)
      _text_selection_event_handle(bridge, event);
   else
     WRN("Unhandled event type: %s", event->desc->name);
}

static void
_bridge_on_connection_lost(void *data, Eldbus_Connection *conn EINA_UNUSED, void *event_info EINA_UNUSED)
{
   Elm_Atspi_Bridge *bridge = data;
   _bridge_disconnect(bridge);
}

static void
_bridge_connection_init(Elm_Atspi_Bridge *bridge, const char *address)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);

   pd->a11y_bus = eldbus_private_address_connection_get(address);
   if (pd->a11y_bus)
     {
        pd->connected = EINA_TRUE;
        _bridge_on_connected(bridge);
        eldbus_connection_event_callback_add(pd->a11y_bus, ELDBUS_CONNECTION_EVENT_DISCONNECTED,
                                             _bridge_on_connection_lost, bridge);
        efl_event_callback_call(bridge, ELM_ATSPI_BRIDGE_EVENT_CONNECTED, NULL);
     }
   else
     ERR("Unable to establish a11y dbus connection");
}

static void
_bridge_a11y_bus_address_get(void *data, const Eldbus_Message *msg, Eldbus_Pending *pending EINA_UNUSED)
{
   ...
   _bridge_connection_init(bridge, sock_addr);
}
...
static void _bridge_object_register(Eo *bridge, Eo *obj)
{
   ...
   // register object and all its descendants
   Eina_List *to_process = eina_list_append(NULL, obj);
   while (to_process)
      {
         Elm_Interface_Atspi_Accessible *o = eina_list_data_get(to_process);
         to_process = eina_list_remove_list(to_process, to_process);
         ...
         _bridge_on_object_registered(bridge, o);
...
static void
_bridge_status_proxy_properties_changed(void *data, Eldbus_Proxy *proxy EINA_UNUSED, void *event)
{
   ...
   if (ev->name && !strcmp(ev->name, "ScreenReaderEnabled" ) &&
       ifc && !strcmp(A11Y_DBUS_STATUS_INTERFACE, ifc))
     {
        ...
        _bridge_enabled_set(bridge, val);
     }
}

static void
_bridge_a11y_stack_enabled_get(void *data, const Eldbus_Message *msg, Eldbus_Pending *pending EINA_UNUSED)
{
   Elm_Atspi_Bridge *bridge = data;
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
   const char *errname, *errmsg;
   ...
   _bridge_enabled_set(bridge, is_enabled);
}

static Eina_Bool
_bridge_platform_a11y_status_monitor_init(Elm_Atspi_Bridge *bridge)
{
   Eldbus_Proxy *proxy;
   Eldbus_Pending *req;
   Eldbus_Object *obj;
   ...
   if (pd->bus_obj) return EINA_TRUE;

   if (!(obj = eldbus_object_get(pd->session_bus, A11Y_DBUS_NAME, A11Y_DBUS_PATH)))
     {
        ERR("Could not get /org/a11y/bus object");
        return EINA_FALSE;
     }
   if (!(proxy = eldbus_proxy_get(obj, A11Y_DBUS_STATUS_INTERFACE)))
     {
        ERR("Could not get proxy object for %s interface", A11Y_DBUS_STATUS_INTERFACE);
        eldbus_object_unref(obj);
        return EINA_FALSE;
     }
   if (!(req = eldbus_proxy_property_get(proxy, "ScreenReaderEnabled",
                                         _bridge_a11y_stack_enabled_get, bridge)))
     {
   ...
   pd->bus_obj = obj;
   pd->stack_status_request = req;
   pd->status_proxy = proxy;

   eldbus_proxy_properties_monitor(proxy, EINA_TRUE);
   eldbus_proxy_event_callback_add(proxy, ELDBUS_PROXY_EVENT_PROPERTY_CHANGED,
                                   _bridge_status_proxy_properties_changed, bridge);
   return EINA_TRUE;
}

static Eldbus_Message*
_bridge_method_call_to_proxy_new(Elm_Atspi_Proxy *proxy, const char *interface, const char *method)
{
   struct dbus_address addr = {{0, }};

   if (!_id_parse(elm_atspi_proxy_id_get(proxy), &addr))
     {
...
static void
_bridge_elm_atspi_socket_embed(Eo *obj EINA_UNUSED, void *data EINA_UNUSED, Elm_Atspi_Proxy *proxy)
{
   ...
   Eldbus_Message *message =
      _bridge_method_call_to_proxy_new(proxy, ATSPI_DBUS_INTERFACE_SOCKET, "Embedded");
   if (!message) return;
...
static void
_bridge_elm_atspi_socket_unembed(Eo *obj EINA_UNUSED, void *data EINA_UNUSED, Elm_Atspi_Proxy *proxy EINA_UNUSED)
{
   // Currently "Unembedded" is not supported by at-spi protocol.
}

static void
_bridge_ref_release(void *data, const void *deadptr EINA_UNUSED)
{
   efl_unref((Eo*)data);
}

static void
_bridge_elm_atspi_plug_embed_by(Eo *obj EINA_UNUSED, void *data EINA_UNUSED, Elm_Atspi_Proxy *proxy)
{
   Elm_Atspi_Bridge *bridge = _elm_atspi_bridge_get();
   ...
   Eldbus_Message *message =
      _bridge_method_call_to_proxy_new(proxy, ATSPI_DBUS_INTERFACE_SOCKET, "Embed");
   ...
   Eldbus_Pending *req = eldbus_connection_send(pd->a11y_bus, message, _on_plug_register, bridge, 100);
   eldbus_pending_data_set(req, "__obj", obj);
   eldbus_pending_data_set(req, "__proxy", proxy);
   if (!req)
     ERR("eldbus_connection_send failed");
   else
     _bridge_pending_add(bridge, req, _bridge_ref_release, efl_ref(proxy));
}

static void
_bridge_elm_atspi_plug_unembed_by(Eo *obj EINA_UNUSED, void *data EINA_UNUSED, Elm_Atspi_Proxy *proxy)
{
   Elm_Atspi_Bridge *bridge = _elm_atspi_bridge_get();
   ...
   Eldbus_Message *message =
      _bridge_method_call_to_proxy_new(proxy, ATSPI_DBUS_INTERFACE_SOCKET, "Unembed");
...
static void
_socket_hooks_uninstall(Elm_Atspi_Socket *socket)
{
...
static void
_plug_hooks_uninstall(Elm_Atspi_Plug *socket)
{
   efl_object_override(socket, NULL);
}
...
static void
_bridge_platform_a11y_status_monitor_shutdown(Elm_Atspi_Bridge *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);

   if (pd->stack_status_request) eldbus_pending_cancel(pd->stack_status_request);
   if (pd->status_proxy)
     {
        eldbus_proxy_event_callback_del(pd->status_proxy, ELDBUS_PROXY_EVENT_PROPERTY_CHANGED,
                                   _bridge_status_proxy_properties_changed, bridge);
        eldbus_proxy_properties_monitor(pd->status_proxy, EINA_FALSE);
...
static void
_bridge_plugs_clean_up(Eo *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
static void
_bridge_connect(Elm_Atspi_Bridge *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
   ...
   Eldbus_Message *msg =
      eldbus_message_method_call_new(A11Y_DBUS_NAME, A11Y_DBUS_PATH, A11Y_DBUS_INTERFACE, "GetAddress");
   if (!msg)
     {
        ERR("eldbus_object_method_call_new failed");
        return;
     }
   pd->connect_request = eldbus_connection_send(pd->session_bus, msg,
                                                _bridge_a11y_bus_address_get, bridge, 100);
...
_bridge_disconnect(Elm_Atspi_Bridge *bridge)
{
   ...
   _bridge_on_disconnected(bridge);
   eldbus_connection_unref(pd->a11y_bus);
   efl_event_callback_call(bridge, ELM_ATSPI_BRIDGE_EVENT_DISCONNECTED, NULL);
}

static void
_bridge_pending_add(Elm_Atspi_Bridge *bridge, Eldbus_Pending *pending, Eldbus_Free_Cb cb, const void *data)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
static void
_bridge_pending_del(Elm_Atspi_Bridge *bridge, Eldbus_Pending *pending)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
static void
_bridge_pending_cancel_all(Elm_Atspi_Bridge *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
static void
_bridge_event_handlers_unregister(Eo *bridge)
{
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
...
/**
 * Function called when bridge disconnects from a11y bus.
 * the pd->a11y_bus variable still contains valid dbus-connection,
 * which will be deleted after function returns.
 */
static void
_bridge_on_disconnected(Elm_Atspi_Bridge *bridge)
{
   ...
   eina_hash_free(pd->state_hash);
}

/**
 * Function called when bridge connects to a11y bus.
 * the pd->a11y_bus variable contains valid dbus-connection.
 */
static void
...
static void
_bridge_enabled_set(Elm_Atspi_Bridge *bridge, Eina_Bool value)
{
   if (value)
     _bridge_connect(bridge);
   else
     _bridge_disconnect(bridge);
}

/**
 * @brief function set new root object for bridge. Root object can be accessed
 * by using well-know at-spi dbus path: /org/a11y/atspi/accessible/root
 */
EOLIAN static void
_elm_atspi_bridge_elm_interface_accessible_observer_on_root_changed(Elm_Atspi_Bridge *bridge, Elm_Atspi_Bridge_Data *pd, Elm_Interface_Atspi_Accessible *new_root)
{
   ...
   Eina_Bool was_connected = pd->connected;
...
static Eina_Bool
_id_parse(const char *id, struct dbus_address *addr)
{
   size_t source_len;
   char *split;
   int to_write;
...
static char*
_id_make(const struct dbus_address *address)
{
   char buf[512];
...
static void
_bridge_plug_register(Eo *bridge, Elm_Atspi_Plug *plug)
{
...
static void
_bridge_plug_unregister(Eo *bridge, Elm_Atspi_Plug *plug)
{
   EINA_SAFETY_ON_NULL_RETURN(plug);
   ELM_ATSPI_BRIDGE_DATA_GET_OR_RETURN(bridge, pd);
   Eina_List *node;
