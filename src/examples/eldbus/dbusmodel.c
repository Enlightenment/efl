                           ...
                           void *event_info)
{
   ...
   eo_do(model, name = eldbus_model_proxy_name_get());
   printf(" -> %s\n", name);
...
main(int argc, char **argv EINA_UNUSED)
{
   ...
   Eo *root = eo_add_ref(ELDBUS_MODEL_OBJECT_CLASS, NULL,
     eldbus_model_object_constructor(ELDBUS_CONNECTION_TYPE_SESSION,
                                     NULL, EINA_FALSE, bus, path));

   eo_do(root, eo_event_callback_add(EFL_MODEL_BASE_EVENT_LOAD_STATUS, _event_load_status_cb, NULL));
   eo_do(root, efl_model_load());
